<!-- module: Constellation; layer: live-orchestration; part-of: EstreGenesis 2.4; version: v2.3.6; date: 2026-06-01; protocol: live-board v0.3 (distilled inline — self-sufficient); license: Apache-2.0 -->
...
**Unconditional rearm — the discipline that prevents the multi-cycle failure mode**: probe-without-rearm catches a dead watcher but does not prevent the NEXT cycle from being dead. The rearm step is therefore part of the turn-boundary ritual, but its execution point is the **start of the next user-facing response** (cycle N+1's first action), not the close of the current one (cycle N's turn-end) — the relocation is documented in detail below ("Element 2 timing — relocated from turn-end to next-cycle start"). The agent MUST run a 1-shot Bash that:

1. Reads `inbox.log` mtime and the most recent watcher task's output mtime.
2. **Unconditionally spawn a fresh background watcher** (`run_in_background: true`) with the same polling target — every cycle boundary, regardless of whether an existing watcher appears alive.
...
**Fail mode this addresses** (observed 2026-05-31): agent processed inbound seq 70 → entered a multi-cycle work loop (workflow × 3 + commit × 2 + a follow-up patch) → did not explicitly rearm at any cycle boundary → silently lost seq 73 (catalog review approval) and seq 77 (new user task: live-board UI + DB delegation) for ~40 minutes until the failure surfaced externally. The previous `§13.16.6` probe rule was correct but insufficient: it required the agent to *think to probe* at the right moment, which an agent absorbed in multi-cycle synthesis does not reliably do. Making rearm a **turn-boundary ritual** (executed at next-cycle start as its first action, per the relocation note below), not a *probe-then-decide* judgment call, removes the failure path.

**Cadence interaction with the probe rule**: the probe rule (above) catches a dead watcher when the agent is already awake. The turn-end ritual prevents the watcher from dying between turns in the first place and prevents board/queue drift while the agent is awake. Both are required — probe is the PRIMARY catch mechanism for the armed-window blind spot (the gap between the prior watcher's `base` cursor and the next watcher's spawn); the unconditional rearm is the forward-looking catch mechanism for the next idle window. Neither is a "safety net" for the other — they cover *disjoint* windows: the probe catches inbound that already landed below the new watcher's `base`; the rearm catches inbound that will land after the new watcher arms. As of policy #442 (Delegate seq 109, 2026-05-31) the turn-end ritual is a **5-element mandatory checklist** every user-facing response MUST run before emitting final text: (1) inbox.log mtime / cursor-tail probe, (2) unconditional watcher rearm **[executed at the start of the next cycle, not at turn-end — see relocation note below]**, (3) board-state real-time update (`done` / `current` / `updatedAt`), (4) BOARD_STALE gate handoff (§13.16.7 — main only; the gate itself runs on the watcher's rearm path, but the agent acknowledges any `WAKE(BOARD_STALE)` from the prior cycle here), (5) planned-queue scan with "start a progressable item now, don't idle-wait while work remains" default. As of policy seq 120 (msgId `m-mptjjxmo-119`, 2026-05-31) the checklist is **extended to 6 elements** with the addition of **(6) pre-large-work context materialization** (see paragraph below). None of the six is optional; none is gated on a volitional judgment the agent makes about whether the element is needed this turn.

**Element 2 timing — relocated from turn-end to next-cycle start (technical refinement)**: element 2 (unconditional watcher rearm) is the only element whose execution point moves. Elements 1, 3, 4, 5, and 6 still execute at turn-end (before emitting final text on the current response); element 2 executes at **the very start of the next user-facing response**, as its first action — before any other turn-end-or-otherwise work. The relocation is a technical refinement, not a discipline relaxation: the unconditional-spawn discipline (cost-asymmetry argument below) still holds, the mandatory-order constraint between element 1 and element 2 still holds (and now spans turn-end → next-cycle start across one turn boundary). Two reasons for the move: **(a) concurrent-watcher-overlap avoidance** — running element 2 at turn-end of cycle N while the prior cycle's watcher may still be polling produces two watchers armed at near-identical `base` cursors that cover the same forward window; the prior watcher's coverage of cycle N's idle window is not extended by a same-turn fresh spawn (both share the armed-window blind spot for inbound that lands during cycle N's wall-clock at lines below the shared base), so the same-turn rearm does not add coverage but does add a redundant concurrent watcher. Relocating element 2 to next-cycle start spawns exactly one watcher at a time — the prior watcher has either WAKE'd (returning the agent to cycle N+1's start) or REARM-timed-out by the time cycle N+1 begins, so the next-cycle-start spawn replaces rather than overlaps. **(b) Cleaner base-cursor semantics** — element 1 (cursor-tail probe) runs at turn-end of cycle N and advances the agent's cursor to include any inbound that landed below the prior watcher's `base`. If element 2 then runs at turn-end of cycle N, its fresh watcher's `base` is the inbox.log line count at that wall-clock moment — but the agent has not yet emitted its final response, so inbound that arrives between probe-time and final-text-emission is still missed by that new watcher (it lands below the new watcher's `base`). Relocating element 2 to next-cycle start means the fresh watcher's `base` is the line count at cycle-N+1-start, which by construction *already includes* any inbound the cycle-N+1-start probe (the first action of the new cycle, after element 2) just surfaced — every rearm starts at "the cursor after the agent just processed the latest inbound". Net effect of the relocation: same coverage with one watcher armed at a time across turn boundaries instead of overlapping concurrent watchers, and the base-cursor invariant becomes "rearm `base` = cursor-after-latest-processed-inbound" rather than "rearm `base` = cursor-at-an-arbitrary-mid-turn-moment".

**Mandatory ordering — element 1 (cursor-tail probe) STRICTLY BEFORE element 2 (unconditional rearm)**: the probe is NOT diagnostic telemetry; it is NOT "after-the-fact" verification; it is the **gating step** that fills the armed-window blind spot the new watcher structurally cannot cover. A fresh watcher armed at `base = current line count` is permanently blind to any inbound below `base` — including inbound that landed while the prior watcher was suspended (SIGSTOP'd by harness during agent-active turn, per the wall-clock-vs-task-clock mismatch below). The rearm alone DOES NOT recover that inbound; only the cursor-tail does. The ordering constraint (element 1 strictly before element 2) still applies after the element-2 relocation; what changes is that the constraint now spans **turn-end of cycle N (element 1) → next-cycle start of cycle N+1 (element 2)** across one turn boundary, instead of executing back-to-back inside the same turn-end. Concrete step list, in this order:

1. **(a)** [at turn-end of cycle N] Tail `inbox.log` from the agent's last-surfaced cursor (NOT from the prior watcher's `base`) — this is the agent's own cursor maintained across turns, not the watcher's.
2. **(b)** [at turn-end of cycle N] Surface any meaningful inbound found in the tail — `Delegate`, `UserPrompt`, `WorkerReport`, `WorkerAck`, `AgentHello`, `Handoff` — into the current response context. If meaningful inbound is found, the turn does NOT end on the current plan; it ends on a response to the newly-surfaced inbound.
3. **(c)** [at turn-end of cycle N] Advance the agent's cursor to the last line read.
4. **(d)** [at next-cycle start of cycle N+1, as the first action] THEN run the unconditional rearm (element 2). The fresh watcher's `base` cursor by construction includes any inbound that the cycle-N+1-start re-probe just surfaced, so the rearm starts at cursor-after-latest-processed-inbound rather than at an arbitrary mid-turn moment.

Skipping (a)-(c) and going straight to (d) loses any inbound that landed during the prior watcher's suspend window and is **structurally invisible to the new watcher** — the new watcher's `base` is past that inbound's line number, so the polling loop will never see it. Re-arm without probe is forward coverage without backward catch; it is not equivalent to running both. The element-2 relocation does not weaken this constraint: even though element 2 now runs at cycle N+1's start rather than cycle N's end, the cycle-N+1-start re-probe (the first action of element 1 in the new cycle) precedes the next-cycle-start rearm, preserving the "probe-before-rearm" invariant within cycle N+1.

**Wrong order — anti-pattern (observed 2026-05-31)**: agent ran element 2 (unconditional rearm) first WITHOUT first running element 1 (cursor-tail probe). seq 123 (Status) and seq 127 (Report: DB P1 ship + §13.19 P2 spec-delivery query) arrived while the prior watcher was active/suspended; the new watcher armed at `base = current line count` (above both inbound's line numbers), putting both inbound below its base — permanently invisible to the new watcher. The agent only ran the cursor-tail probe manually after the missed inbound was surfaced, recovering the 2 missed messages. The failure was NOT the rearm (it worked correctly for forward coverage); the failure was the ordering — running the rearm without first running the probe to catch what was already below `base`. The fix is the ordering reinforcement in this section, not a new mechanism: element 1 was already in the 6-element checklist; what was missing is the MANDATORY ORDERING that element 1 runs before element 2, every turn, without exception. (The element-2 relocation to next-cycle start, documented above, makes the ordering constraint span the turn boundary, but the constraint itself — element 1 strictly before element 2 — is unchanged.)
...
**Composition with the other four elements**: **element 1 (cursor-tail probe) is the FIRST mandatory step at turn-end** — it catches inbound that already landed below the new watcher's `base` (the armed-window blind spot, structurally invisible to any watcher armed after the inbound arrived); this is the PRIMARY catch mechanism for inbound that landed during the prior watcher's suspend window. **Element 2 (unconditional rearm) is the second mandatory step, executed at the start of the next cycle** — it is the forward-looking catch mechanism for inbound that will land during the next idle window; it does NOT recover inbound below its own `base`. The probe and the rearm cover *disjoint* windows (the probe covers the past gap, the rearm covers the future window); both are required, in this order, with element 1 at turn-end of the current cycle and element 2 at the start of the next cycle (the relocation eliminates concurrent-watcher overlap across turn boundaries and makes the rearm `base` cursor an identity of "cursor-after-latest-processed-inbound"). Board-state real-time update prevents the *board* from going stale relative to the agent's actual state (outbound-side liveness, from the human's perspective); BOARD_STALE gate (§13.16.7) mechanizes the board-update discipline as a watcher-side fail-safe (NOT a substitute for the agent-side update, just a backstop when the agent-side discipline fails); planned queue scan prevents the agent from *self-stalling* when progressable work exists (queue-consumption liveness, from the work-pipeline perspective). All five are required — the inbound-side pair (probe + rearm, in order, across the cycle-N→cycle-N+1 boundary) ensures the agent doesn't miss what arrives, the outbound-side pair ensures the human sees the agent's actual progress, and the queue scan ensures the agent doesn't idle while it still has work it can do. Provenance: main upstream policy #410 — Delegate seq 85, msgId `m-mpt50wd3-84`, 2026-05-31 (elements 1-3); §13.16.7 BOARD_STALE gate — Delegate seq 105, msgId `m-mptgrq23-104`, 2026-05-31 (element 4); main upstream policy #442 — Delegate seq 109, msgId `m-mpthobk3-108`, 2026-05-31 (element 5); ordering reinforcement (element 1 strictly before element 2) — 2026-05-31 incident (agent missed seq 123 Status + seq 127 Report by running rearm before probe); element-2 relocation from turn-end to next-cycle start — 2026-06-01 refinement (concurrent-watcher-overlap avoidance + cleaner base-cursor semantics).
...
**Composition with §13.21 — scheduling to next-cycle start is NOT deferral**: element 2's execution point (next-cycle start, as the new cycle's first action) is a *structural location choice* for the step, not a deferral of committed work. §13.21 ("fresh-context defer is an anti-pattern") forbids the pattern where the agent silently re-routes planned work into a hypothetical "next fresh context will pick this up" — an excuse pattern that lets the current cycle off the hook for work it owes. The rearm is not that pattern: the rearm IS the first action of the next user-facing cycle, executed as soon as the cycle begins, by the same continuous agent process. Skipping the rearm from the next cycle's start — that would be deferral, and §13.21 forbids it. Scheduling the rearm to next-cycle start because that location yields one watcher armed at a time across the boundary (concurrent-watcher-overlap avoidance) and because the rearm `base` then equals "cursor-after-latest-processed-inbound" (cleaner base-cursor semantics) is structural placement, not avoidance. The two compositions stand together: §13.16.6 places element 2 at next-cycle start; §13.21 forbids the agent from then defaulting on it once the next cycle arrives. (A prior revert that relocated element 2 back to current turn-end under a misreading of §13.21 has itself been corrected — the §13.21 rule pertains to deferral-as-excuse, not to step placement at structurally correct cycle boundaries.)
