...
This is unconditional — even if a watcher was launched earlier in the same conversation, re-verify and re-arm at every turn-end. A watcher spawned 30 minutes ago in this conversation has no liveness guarantee at this turn-end; the conversation may have stalled, the harness may have GC'd the background task, the user may have interrupted, the rearm chain may have broken — none of these surface as errors to the agent.
...
**Caveat — harness GC risk on bash background watchers**: bash background tasks spawned via `run_in_background: true` may be silently garbage-collected by the harness shortly after spawn, producing an abnormal-exit signature (observed: 38-byte output = re-armed header only, no `WAKE`, no `REARM`) within ~5 seconds of launch. The condition is not fully understood — some bg tasks survive to wake normally on real inbound (e.g. `b1rrhbwo4`, `bfnmb8e09` on 2026-05-31), while others spawned in adjacent turns die immediately (e.g. `bc6tfgw2d` at 11:25 + `bd48j8vrj` at 11:34 on 2026-05-31, both 38-byte exits). The failure mode appears tied to harness GC of bg tasks scoped to a prior turn, but the trigger is not deterministic from the agent's side. **The agent cannot fully prevent this** — the inbox-mtime probe + turn-end rearm rules above are the *safety net*, not the *cure*. Treat every bg watcher as potentially dead until a fresh probe confirms it.

**Mitigation 1 — redundant watchers per turn-end**: at turn-end, spawn **two** bg watchers with slightly different polling cadences (e.g. 5s + 7s) rather than one. First watcher to wake on a real inbound wins and surfaces the event; the other ages out on its own rearm ceiling. Redundancy turns the GC-kill into a survivable single-point failure: P(both die in the same turn window) is materially lower than P(one dies), and the cost (a second bg slot + a few extra log lines) is small relative to the silent-stall blast radius. Mitigation 2 — external cron fallback: where the harness exposes a scheduling primitive (e.g. Claude Code `CronCreate`), schedule a self-cron every 15-30 min as a coarse-grained safety net. Cron does NOT replace the bash bg watcher — the cron interval is too long for near-real-time inbound — but it catches the *silent-stall class* when both bg watchers die between turns. Surfacing incident: 2026-05-31 user message `"재무장이 또 풀렸네"` — diagnosis traced `bc6tfgw2d` + `bd48j8vrj` to 38-byte abnormal exit; seq 85 went unread for ~30 min because the watcher in that window had died and no cron fallback existed.
