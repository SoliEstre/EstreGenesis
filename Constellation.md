...
**The gate** (mechanization of §13.16.6's third turn-end ritual element — board state real-time update): at the *start* of every watcher re-arm cycle, before entering the polling loop, the watcher compares the latest git HEAD commit epoch on the working tree against the `updatedAt` timestamp written into the live-board `state.json`. If HEAD is more than **120 seconds newer** than `state.json.updatedAt` — i.e. the agent has committed work that the board has not yet been told about — the watcher does NOT re-arm. Instead it exits immediately with a `WAKE(BOARD_STALE)` event surfaced into the watcher's task output (and into `inbox.log` as a self-emitted wake marker, per the §13.16.6 self-emission filter discipline). The next turn-start will read the wake, the agent will write the missing `state.json` deltas, and only then will a fresh watcher arm.
...
**Scope — main only**: the gate runs only when the watcher's `agentId === WS_PRIMARY_AGENT` (i.e. the main agent that owns the live-board `state.json` write). Workers do NOT own `state.json` — their outbound is `WorkerReport` to the main inbox, and the board reflects worker progress via main's relay write — so a worker watcher's HEAD-vs-`updatedAt` comparison would be meaningless (workers commit to their own worktrees on their own cadence, unrelated to the board). Worker watchers skip the BOARD_STALE check and re-arm normally.

**Threshold rationale — 120 s**: the gate compares HEAD's commit epoch (committer-date Unix seconds) to `state.json.updatedAt` (ISO-Z parsed to Unix seconds). 120 s is the margin for a normal `commit → push → state.json write` pipeline: commit hooks (~5-15 s), push to remote (~5-30 s depending on network), `state.json` write (~1 s). Tighter thresholds (30-60 s) risk false positives on slow networks or hook-heavy commits; looser thresholds (300 s+) defeat the purpose by allowing the same multi-cycle drift the gate exists to prevent. The threshold is configurable via env (`CONSTELLATION_BOARD_STALE_THRESHOLD_S`, default `120`) for adopters with markedly slower or faster commit pipelines, but adopters should treat the default as the well-tested value.
