...
11. **자율 실행 (절대)** — 정해진-다음-단계는 묻지 말고 순서대로 진행. 게이트는 오직: (a) 손실 / 외부 발행 (push · deploy · send · delete), (b) 새 중대 분기 결정 시점 (RRP / 설계 — *결정 시점* 만; 그 결과로 잡힌 `Phase A/B/C` 계획은 *결정된 실행*이라 재게이트 아님), (c) 재기동 필요 deploy (적용은 자율, *재기동 타이밍* 만 조율), (d) 명시적 사용자 steering. 정해진-다음-단계를 멈춰 묻는 것 자체가 자율 운영 위반. **비-분기 선택**: 추천안 있음 + 비-분기(정해진 작업의 우선순위 정렬 / sequential-vs-parallel / 거의 동등한 옵션 간 선택) = 1줄 announce 후 추천안대로 진행; 구조화된 옵션 목록(A/B/C/D?)으로 **escalate 하지 말 것**. 병렬 가능 시 **parallel 이 기본 추정**. 사용자 pivot 은 가능하지만 **요청하지 않음**. 턴 종료 §13.16.6 ritual (6 요소) 은 이 원칙의 일부 — planned 스캔 없는 re-arm = 관성. #442 planned 큐 스캔 규칙은 `fresh context 기대` / `compact 임박` 으로 **면제되지 않음** — context 길이 핑계는 §13.21 안티패턴; 즉시 진행 가능한 planned 항목은 perceived context 예산 무관 현재 cycle 에서 시작 (6번째 요소 — 대형 작업 진입 전 context 물질화 — 가 mid-cycle compact 를 survivable 하게 만듦; seq 120 / m-mptjjxmo-119). `Constellation.md` §13.18 + §13.21 참조.
...
**Watcher liveness probe (Constellation.md §13.16.6)** — 외부 응답 대기(원격 `Delegate` 회신, upstream 워커 job-done, inbox rearm cycle) 상태에 들어가면, **launched 된 watcher가 곧 살아있는 watcher가 아님**. watcher 는 예상보다 자주 silent 하게 죽음(crash, 사용자 중단, rearm-ceiling 초과, harness GC) — 응답은 inbox 에 도착하지만 surface 안 됨, 에러 없이 작업 정지. 사용자가 먼저 알아챔(`"응답 왔어?"`). 규칙: 대기 중 매 독립 작업 cycle 마다(idle 시 최소 ~30분 wall-clock 마다), `inbox.log` mtime vs 현재 wall-clock 그리고 watcher task 출력의 최근 `watcher re-armed @ <Z>` 마커가 예상 rearm 간격 이내인지 검증. 둘 중 하나라도 stale = watcher dead 처리 + 명시적 re-arm(새 task ID 기록, 옛 ID dead 표시) + 사용자에게 즉시 staleness 보고. todo state(`[in_progress] watcher`)와 실제 task liveness 혼동 금지. **턴 종료 직전 무조건 rearm** — 매 응답 종료 직전(최종 텍스트 emit 직전), apparent active-liveness 여부와 관계없이 **무조건 새 background watcher spawn** (false-positive spawn 비용 < false-negative miss 비용). probe-and-decide 판단이 아니라 *턴 종료 ritual* 로 만들어야 함 (multi-cycle 에이전트는 정확한 시점 probe 판단을 reliably 못함). **inbox cursor-tail probe 는 rearm 전에 반드시 실행 (매 턴 종료마다)** — 마지막 surface 된 cursor 부터 tail, 의미있는 inbound 가 있으면 surface, cursor 전진, 그 다음 무조건 rearm. probe 는 직전 watcher 의 SIGSTOP-suspend gap 을 잡는 1차 수단 (telemetry 아님). probe skip → inbound 유실. 사용자 surface 2026-05-31 incident anchor. **Planned 큐 스캔 (턴 종료 5번째 요소, 메인 #442 / Delegate seq 109 / msgId `m-mpthobk3-108` / 2026-05-31)** — inbox probe + watcher rearm + 보드 갱신 + BOARD_STALE 게이트와 함께, 턴 종료 시 planned/pending 큐 스캔 → 진행 가능한 항목은 지금 즉시 시작; idle wait 는 진정한 exhaustion(큐 비었거나 모든 항목이 에이전트가 해결 못하는 외부 의존에 `blocked` — 자율 진행 가능한 `planned` 와 구분)에만 예약. **대형 작업 진입 전 context 물질화 (턴 종료 6번째 요소, seq 120 / msgId `m-mptjjxmo-119` / 2026-05-31)** — 대형 multi-cycle 작업(여러 파일 cascade, 대규모 refactor sweep, 멀티 티어 seed sync, 긴 delegation 체인) 진입 전 scope/파일/앵커를 durable artifact(RRP / PM/NNN_*.md / WORKLIST.md / 라이브 보드 `state.json`)에 물질화 → post-compact pickup 가능; in-memory plan 은 compact 를 survive 하지 못함. **Fresh-context defer = 안티패턴 (§13.21)** — `/compact` 재량 부존재 + compact 는 임계치에 따라 fire + fresh 기대는 wishful; 6번째 턴 종료 ritual 요소가 mid-cycle compact 를 survivable 하게 만듦, defer 는 옵션 아님. Provenance: seq 120 / m-mptjjxmo-119. **보드 state 실시간 갱신 (턴 종료 ritual 세 번째 요소)** — inbox probe + watcher rearm 과 함께, 최종 응답 emit 직전 라이브 보드 `state.json` 이 방금 완료한 작업을 반영하는지 검증(done 카운터 tick, current 포인터 진행, 최상위 `updatedAt` 을 현재 wall-clock ISO-Z 로 bump); 보드 `state.json` 은 보드를 보고 있는 사용자 입장의 시각화 SSoT이며, main이 multi-cycle 작업 중 이 실시간 갱신을 누락한 것이 관찰됨(메인 upstream 정책 #410 — Delegate seq 85, msgId `m-mpt50wd3-84`, 2026-05-31). **BOARD_STALE 게이트 (§13.16.7)** — rearm 직전 보드 신선도 검사: git HEAD 커밋 시각 vs 마지막 state.json 쓰기 git 커밋 시각; HEAD 가 state-commit 보다 새로움 → 턴 종료 rearm 전 `WAKE(BOARD_STALE)`. identity 기반(시간 버퍼 / 120s 임계치 없음 — offset 게이트는 self-trap). main 전용; 워커 면제. Provenance: 메인 upstream #434 + seq 103 ORIGINAL + seq 105 CORRECTED (head-vs-state-commit-time) / msgId `m-mptgrq23-104` / commit `694cb20` / 2026-05-31. 전체 discipline: `Constellation.md` §13.16.6.

**Deadlock 해소 (§13.19)** — wait-pattern 분류(strict / quasi / healthy) + 4단계 prevention/detection/auto-resolution/escalation + 3계층 ack(§13.13 위 transport → commitment → application). Provenance: 메인 upstream RRP + EG draft v0.1 (82ab55e) 공동 형식화 seq 111.

**Blocker tracking + nudge (§13.20)** — idle wait 진입 전 `BlockerManifest` emit; rearm cycle 당 주기적 `BlockerNudge` + 4단계 escalation (polite → explicit + ETA → deadline → human §13.17). §13.19 cyclic-deadlock 의 linear-stalled-blocker 짝.

**메인-챗 구조화 선택 프롬프트 금지 (Constellation.md §13.17)** — 메인-챗에 인라인 `AskUserQuestion` 류 옵션 UI는 보드 projection이 없어 멀티-에이전트 가시성을 깨뜨림 → 구조화 Q/A는 보드로 라우팅(defer-OK = review-items 탭; need-now = 라이브-보드 실시간 챗 + 옵션 UI, 신규 UI6 대기 중). 메인-챗의 자유형 텍스트 질문은 OK. UI6 도착 전 과도기 fallback = defer-OK + 보드에 `WorkerReport` "사용자 입력 대기" 1줄. **review-items 탭 뱃지** — 탭은 nav의 상수 요소로 **항상 표시**, 뱃지는 `count > 0` 일 때만 카운터 표시 `count === 0` 이면 숨김 (`app.js` `badge.hidden` 토글; `style.css` `[hidden] { display: none }` 강제) — 탭 항상 표시 = defer-OK 경로 가용 신호, 0일 때 뱃지 숨김 = 시각적 잡음 제거; 향후 보드 재구현은 두 절반 모두 보존 (뱃지 UI: 메인 Delegate seq 113, msgId `m-mptiputv-112`). Provenance: 메인 #414 (Delegate seq 88, msgId `m-mpt5o07l-87`, 2026-05-31). 본문: `Constellation.md` §13.17.

**EstreUX (Estre Universal eXpression) — `.eux` brew 런타임** — [EstreUX](https://github.com/SoliEstre/EstreUX) 는 `.eux` spec 포맷의 brew 런타임 (Apache-2.0, v0.1.0+; EstreGenesis 와 별도, 비번들 참조 런타임). 이름 분해: `Estre + U + eX` → `.eux` = **Estre Universal eXpression** — *Universal* 인 이유는 포맷이 UI 전용이 아니기 때문 (2026-05-30 downstream dogfood 가 UI · backend · protocol · state-machine · mobile-app 전반에서 ~93% line-fidelity 증류 검증 — `Constellation.md` §6 + `Superscalar.md` §11 Entry 04), *eXpression* 인 이유는 *코드 ↔ spec* 양방향 증류 포맷이지 UI experience 레이어가 아니기 때문. **도입 시점**: 멀티 스택(예: `.cjs` + `.py`)에서 같은 `.eux` 를 각 타깃으로 brew; UI/backend/protocol 컴포넌트의 spec ↔ code drift 추적; `Constellation.md` §6 의 라이브 보드 컴포넌트가 distillable 시작점. **Skip when**: 1-shot · 단일 스택 · spec 작성 오버헤드가 round-trip 가치 초과 — 평문 코드/마크다운으로 충분. **Brew (lite, no full clone)**: `npx giget gh:SoliEstre/EstreUX/spike#v0.1.0 ./estreux-engine` → `node ./estreux-engine/expand.mjs brew <comp>.eux`. Full-clone 대안: `node bin/estreux.mjs brew|drift <comp>.eux`. npm `estreux` 패키지는 publish-ready (릴리스 대기). **출처 인용 규율** — 커밋·문서·외부 발행물에서 EstreUX 또는 `.eux` 를 거명할 때 전체 이름 + 공개 repo 인용: `EstreUX (Estre Universal eXpression, https://github.com/SoliEstre/EstreUX)`. `.eux` 확장자의 원천은 EstreUX 이므로 `.eux` 파일도 동일 attribution 상속. Constellation.md / EstreGenesis 자체 인용과 동일한 external-public-framework attribution 규율.
