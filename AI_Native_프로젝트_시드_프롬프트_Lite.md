...
**운영 규율 — 인라인 아닌 포인터 (Constellation.md §13.x)** — 전체 본문 SSoT 는 `Constellation.md` §13.x; 시드는 invariant 어휘 + 1줄 요지만 유지. **§13.11 보드 emission**: safe point 마다 진행 emit 의무 · 무활동 시 자율 heartbeat 금지 (false-alive). **§13.13 A2A ack 계층**: 송달 3등급 `delivered`/`read`/`processed` (ROGER ≠ WILCO) · 어휘 `msgId`(dedup watermark) / `Ack{delivered}`(자동, ack 은 ack 안 함) / `AckProcessed` / `AckCumulative` / `Ping/Pong`(liveness probe — 재전송 도구 아님) · ack timeout 시 보수적 Ping → inbox dedup 조회 → 유실만 재전송 → 사람 보고 (Two Generals 종결) · server/bridge/agent 계층 분리. **§13.16.6 턴 종료 ritual (6요소, 순서 고정)**: ① inbox cursor-tail probe (rearm 전 필수) ② 무조건 watcher rearm (launched ≠ live) ③ 보드 state 실시간 갱신 (done tick · current 진행 · `updatedAt` bump) ④ BOARD_STALE 게이트 (§13.16.7, HEAD vs state-commit identity 비교, main 전용) ⑤ planned 큐 스캔 (blocked/planned 구분, 진행 가능하면 즉시 시작) ⑥ 대형 작업 진입 전 context 물질화 (durable artifact → post-compact pickup). **§13.17 메인-챗 구조화 선택 프롬프트 금지**: 인라인 옵션 UI 는 보드 projection 없음 → defer-OK = review-items 탭 / need-now = 라이브-보드 챗 + 옵션 UI; 자유형 텍스트 질문 OK; 탭 항상 표시 + 뱃지 count>0 만. **§13.19 deadlock 해소**: wait-pattern strict/quasi/healthy 분류 + prevention/detection/auto-resolution/escalation 4단계 + 3계층 ack (transport → commitment → application). **§13.20 blocker tracking + nudge**: idle 진입 전 `BlockerManifest` emit + 주기적 `BlockerNudge` 4단계 escalation — §13.19 의 linear-stalled 짝. **§13.21 fresh-context defer 안티패턴**: planned 작업의 다음-세션 연기 금지; ⑥ 물질화가 mid-cycle compact 를 survivable 하게 만듦.
