...
개발자가 upstream repo를 운영하거나 수정 권한이 있고, 현재 project에서 upstream-bound 변경을 먼저 구현하거나 테스트할 때 사용합니다. EstreUI 전용이 아닙니다.
...
EstreUI 적용 host project는 user-named upstream folder를 사용하는 upstream split을 적용했습니다: `.agent/estreui/` + `.agent/project/`. 이는 구체적 사례이지 일반 기본값이 아닙니다. 일반 project는 사용자가 더 명확한 upstream 이름을 고르지 않는 한 `.agent/upstream/`을 사용해야 합니다.

## 리버스 참조

- EstreUI upstream split 사례: bundle 작성자의 private host project 및 public EstreUI repo context.
