...
**입력**: 재발행된 브리핑에 대해 다시 한국어 재작성 요구 — 연속 2회차 fallback 트리거.
...
**입력**: 권장안 채택. 추가로 Constellation 가동 상태면 검토사안 패널에 올려 지연 검토·결정이 가능하게 하고 알림에도 그 경로를 명시. 미가동이면 검토사안 문서를 만들어 둘지 묻거나, 설정에 따라 항상 자동 생성하는 방식 제안.
