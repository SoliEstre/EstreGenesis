...
**입력**: 한국어 재작성 요구 — 3회차 fallback 트리거.
...
**입력**: 「1.0 정식 출시」라는 표기가 상당히 자의적이므로 그 기준을 먼저 세우는 것이 선행. 당장은 현 상태로 두되, 1.0 을 10점으로 놓았을 때 현재 수준이 몇 점인지 평가가 필요하다는 판단.
...
**입력**: 1.0 기준으로서는 문제없으나, 현 안건은 Claude 용 플러그인 게시 기준이므로 다른 도구에서의 사용은 «공식 마켓 등록 판단» 평가에서만 제외하는 것이 맞다는 판단.
