...
<!-- seed-tier: Compact; language: English; version: v1.6.0; date: 2026-05-09; counterpart: AI_Native_프로젝트_시드_프롬프트_Compact.md; changelog: upstream EstreGenesis repository README.md, not target project README.md -->
...
- **`AGENTS.md`**: SSoT. Sections: (1) reading order with `<scope-root>` · (2) roles · (3) bridges per P4 · (4) coordination protocol matching § Multi-agent cadence · (5) core rules incl. items 7–13 (index sync · N-way sync · `~` escape · RAG density · hooks · repo residency · agent-time estimation w/ pace mode) · (5.8) N-way sync registry stub · (7) commit format · (8) refs. Core rules line 1 records language + tone + pace mode together.
