...
<!-- seed-tier: Compact; language: English; version: v1.4.0; date: 2026-05-03; counterpart: AI_Native_프로젝트_시드_프롬프트_Compact.md; changelog: README.md -->
...
- **P7 Execute** — (A) scaffold all selected files (see § Scaffold specs), (B) intermediate+ writes `.agent/PM/001_Phase1_Plan.md`, (C) advanced+ pins versions in `.agent/architecture.md` + data flow, (D) expert writes MVP WBS + risk register.

## Scaffold specs (P7-A — generate per spec, the agent writes the actual file)

- **`AGENTS.md`**: SSoT. Sections: (1) reading order · (2) roles · (3) bridges per P4 · (4) coordination protocol matching § Multi-agent cadence · (5) core rules incl. items 7–10 (index sync · N-way sync · `~` escape · RAG density) · (5.8) N-way sync registry stub · (7) commit format · (8) refs.
- **`.agent/rules.md`**: language/docs/git/coordination/troubleshooting policy stub matching the items in AGENTS.md §5.
- **`.gitignore`**: Common (OS: `.DS_Store` `Thumbs.db` `desktop.ini`; IDE: `.idea/` `.vscode/*` with sub-allowlist; env: `.env*` `!.env.example` `*.pem` `*.key` `secrets/`; logs: `*.log` `npm-debug.log*`) + Phase-2-stack rows (Node: `node_modules/` `dist/` `.next/` `.nuxt/` `.turbo/` `coverage/`; Python: `__pycache__/` `.venv/` `*.egg-info/` `.pytest_cache/` `.coverage`; Go: `bin/` `*.exe` `vendor/`; Rust: `target/` `**/*.rs.bk`; JVM: `*.class` `.gradle/` `build/`; DB: `*.sqlite` `*.db`) + Seed-produced artifacts (`.agent/_questions/open/*.draft.md`, `.agent/scratch/`, `**/*.generated.html`, `**/*.generated.pdf` — comment out the last two if the project commits PDFs e.g. external-delivery packets).
...
- **Bridge stubs (per P4 selection)**: each bridge is a 3–6 line file naming SSoT (`AGENTS.md`), the must-read order (AGENTS.md → rules.md → architecture.md → STATE.md → _contracts/), the HANDOFF.md claim rule, and the working language/tone. Service-specific knobs (Claude `.claude/rules/`, Cursor MDC frontmatter, Aider `read:` list, Continue `rules:` list) stay in the bridge.
...
2. **Diff capabilities** — typical deltas: `.agent/_coordination/` (v1.0), research loop (v1.1), migration guides (v1.2), Bootstrap/Migration mode branching (v1.2), Core Principles 6/7/8/9 + § Index Sync · § N-way Sync · § Markdown `~` Escape · § RAG Index Optimization · § Document Inflation Prevention (v1.3), enforcement hooks (v1.3.4), task decomposition strategy (v1.3.5), external knowledge index auto-sync (v1.3.6), Phase 0 agent tone selection (v1.3.7). Present as numbered menu.
