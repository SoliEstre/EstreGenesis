...
**v0.4.0 status (2026-06-03 update)**: Phase 2 renderer가 ship 되었습니다 — `plugins/hyperbrief/renderers/mini-engine.cjs` (자체 mini-engine + ajv schema validation + 톤 3축 transform + 결정성 보장) + `bin/render.cjs` (CLI 진입점) + `types.d.ts` (인터페이스 선언) + `package.json` (`ajv ^8.17.0` 단일 의존). MCP tool 노출 (`hyperbrief_render`)은 v0.4.1 deferred (별도 cut). 위 transition rules 본문은 **historical record로 유지** (deprecate하지 않음) — 이유: (a) renderer가 ship 되었어도 외부 adopter가 동시에 옮겨가는 것이 아니므로 fallback discipline 가치 지속, (b) Phase 1 운영 경험이 본문에 캡처된 채로 남아야 향후 schema 진화 시 갱신 근거가 명확. **v0.4.0 이후 신규 brief**는 mini-engine renderer 사용 SHOULD; renderer 부재 환경 (Node <18, ajv 미설치 등) 에서만 template-substitute fallback. drift-log의 새로운 의미: Phase 2 renderer 산출과 Phase 1 template-substitute 산출 간 hash 대조로 backwards-compat 회귀 검증 contract.
...
| 02 | `hb-20260603-r2nd02` | 2026-06-03 | `one_way_with_migration_path` 🟡 | accept (대안 B — 자체 mini-engine + ajv + CLI; MCP tool 노출은 v0.4.1 deferred) | _pending revisit 2026-09-01_ | `_proposals/006_2026-06-03_hyperbrief/dogfood-entry-02.md` |
