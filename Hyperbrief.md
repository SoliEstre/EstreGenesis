<!-- module: Hyperbrief; layer: decision-gating; part-of: EstreGenesis 2.5.x; version: v0.5.6; date: 2026-06-03; status: design draft v0.5.6 (§5.6.7 + MUST-19 auto-localize discipline — skill MUST populate audience_profile_fallback.button_label + trigger_phrases_md in user's prevailing conversation language at IR-emit time; schema Korean defaults are last-resort fallback for IR-direct-call/test only; reference literals EN/KO/JA in §5.6.7; SKILL.md updated; v0.5.5 §11.5 v1.0 readiness rubric Lens A 5.3/4.9 + Lens B 5.5/5.1 — both sub-threshold — unchanged); depends-on: none (optional synergy: Constellation §13 A2A — active, Superscalar §3.1 decision-delegation interlock — active); license: Apache-2.0 -->
