<!-- module: Hyperbrief; layer: decision-gating; part-of: EstreGenesis 2.5.x; version: v0.4.2; date: 2026-06-03; status: design draft v0.4.2 (Phase 2 MCP exposure shipped — plugins/hyperbrief/mcp/server.cjs with 4 tools hyperbrief_render / hyperbrief_validate / decision_ledger_append / decision_ledger_query, plus Constellation a2a_wait_ack tier='decided' recognition. Constellation §13.16.9 A2A-intent family 5 names + ack_tier='decided' shipped at Constellation v2.3.20. §11.1 v0.2.0 and the v0.4.2 MCP-noth items both shipped.); depends-on: none (optional synergy: Constellation §13 A2A — active, Superscalar §3.1 decision-delegation interlock — active at Superscalar v0.4.1); license: Apache-2.0 -->
...
- **v0.5.0** — schema v0.5 candidates (declared vs effective `audience_profile` `surface_profile_estimate`, telemetry-driven `trigger_phrases_md` learning, `recommended_artifacts.body` sub-schema for structured code artifacts) + dogfood ledger §11 Entry 03-05 (real cases with retention of user_acceptance_rate, Brier score, pre-mortem text length distribution).
