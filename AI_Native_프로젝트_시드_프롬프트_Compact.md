...
<!-- seed-tier: Compact; language: Korean; version: v2.1.0; date: 2026-05-28; counterpart: AI_Native_Project_Seed_Prompt_Compact.md; changelog: upstream EstreGenesis repository README.md, not target project README.md -->
...
14. 라이브 오케스트레이션 (Constellation): 멀티에이전트 코디네이션을 파일 기반(`.agent/_coordination/`)에서 실시간 라이브보드(WS + A2A) + 대시보드로 격상. **A2A 브릿지 인터페이스 (불변부)**: role board/main(오케스트레이터·타깃 미지정 수신)/local(워커)/upstream(`uk-` 키)/collab(`ck-` 키 + join URL); 핸드셰이크 WS→SERVER_HELLO→`HELLO{agentId,role}`→A2A `AgentHello{targetAgentId:main}`→OnboardAck→Delegate 대기; 워커 `WorkerReport`, 보드 SSoT=메인; turn 기반 에이전트(Claude Code) = 브릿지 데몬(파일 IO inbox/outbox) + self-wake watcher, detached 상주. 인라인 아닌 참조: `Constellation.md` + `constellation/*.eux` raw URL (`raw.githubusercontent.com/SoliEstre/EstreGenesis/main/Constellation.md`; 재현성 tag 핀). brew 런타임 = EstreUX (https://github.com/SoliEstre/EstreUX). UI 컴포넌트는 `.eux` 로 작성해 EstreUX 로 brew — 별도 참조 런타임(포맷 SSoT = EstreUX `docs/eux-format-v0.md` / `BREW.md`), 본 시드가 소유하는 기능 아님. 깊이는 시드 티어 따라감. 선택적.
...
- **`AGENTS.md`**: SSoT. 섹션: (1) `<scope-root>` 를 포함한 읽기 순서 · (2) 역할 · (3) 브릿지 (P4 결과) · (4) 코디네이션 프로토콜 (§ 멀티에이전트 cadence 와 일치) · (5) 핵심 규칙 — 항목 7-14 (인덱스 동기화 · N-way sync · `~` escape · RAG 풍부성 · 훅 · repo residency · 페이스 모드 포함 agent-time 추정 · Constellation) 포함 · (5.8) N-way sync 등록부 stub · (7) 커밋 형식 · (8) 참조. 핵심 규칙 라인 1 은 언어 + 말투 + 페이스 모드 함께 기록.
