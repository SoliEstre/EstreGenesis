...
<!-- seed-tier: Compact; language: Korean; version: v2.4.3; date: 2026-06-03; counterpart: AI_Native_Project_Seed_Prompt_Compact.md; changelog: upstream EstreGenesis repository CHANGELOG.md, not target project README.md; v2.4.3 = main hub feedback reflection — 마이그레이션 B 1-liner 에 v2.3.0 원칙 #15/#16 (Superscalar + 자율 실행) 명시 추가 -->
...
14. 라이브 오케스트레이션 (Constellation): 멀티에이전트 코디네이션을 파일 기반(`.agent/_coordination/`)에서 실시간 라이브보드(WS + A2A) + 대시보드로 격상. **A2A 브릿지 인터페이스 (불변부)**: role board/main(오케스트레이터·타깃 미지정 수신)/local(워커)/upstream(`uk-` 키)/collab(`ck-` 키 + join URL — `OnboardAck` role 분기 §13.9: collab/upstream = peer, 워커 아님); 핸드셰이크 WS→SERVER_HELLO→`HELLO{agentId,role}`→A2A `AgentHello{targetAgentId:main}`→OnboardAck→Delegate 대기; 워커 `WorkerReport`, 보드 SSoT=메인; turn 기반 에이전트(Claude Code) = 브릿지 데몬(파일 IO inbox/outbox) + self-wake watcher, detached 상주. **§13.11 보드 emission 규율** (safe point마다 진행 emit 의무 · 무활동 시 자율 heartbeat 금지 — `codex-watch.cjs` 제거). **§13.13 A2A ack 계층** (3 송달 등급 delivered/read/processed · `msgId` bridge 자동 dedup · server 자동 `Ack{delivered}` 보드 미표시 alarm-fatigue 게이팅 · 선택 `AckProcessed` WILCO · `AckCumulative` telemetry · Ping/Pong은 application liveness probe — **재전송 아님**; ack timeout 시 보수적 Ping → inbox dedup → 유실만 재전송 → 무응답 지속 시 사람 보고 Two Generals). **§13.16.6 watcher liveness probe** — 외부 응답 대기 상태에서 *launched ≠ live*: watcher 는 silent 하게 죽음(crash · 사용자 중단 · rearm-ceiling 초과 · harness GC). 매 cycle(idle 시 ~30분)마다 probe: `inbox.log` mtime AND watcher rearm 마커; 둘 중 stale = 명시적 re-arm + 사용자에게 즉시 보고. **턴 종료 직전 무조건 rearm** — 매 turn-end **apparent active-liveness 무관 무조건** spawn (probe = telemetry 일 뿐 게이트 아님; probe-and-decide 아닌 *턴 종료 ritual*) + cursor-tail probe 는 rearm 전 (필수 순서 — probe = armed-window blind spot 1차 수단, telemetry 아님; skip 시 이번 세션 seq 123/127 유실) + planned 큐 스캔 (5번째 요소 — blocked 아니면 planned 지금 시작, idle 은 진정한 exhaustion 에만; 메인 #442 / seq 109 / msgId `m-mpthobk3-108` / 2026-05-31) + 대형 작업 진입 전 context 물질화 (6번째 요소 — 대형 multi-cycle 작업 진입 전 scope/파일/앵커를 durable artifact RRP/PM/WORKLIST/state.json 에 물질화하여 post-compact pickup 가능하게; in-memory plan 은 compact 를 survive 못함; seq 120 / msgId `m-mptjjxmo-119` / 2026-05-31). todo `[in_progress]` ≠ 실제 task liveness. **보드 state 실시간 갱신 (턴 종료 ritual 세 번째 요소, 메인 upstream #410 / Delegate seq 85 / msgId `m-mpt50wd3-84` / 2026-05-31)** — inbox probe + watcher rearm 과 함께, 최종 응답 emit 직전 라이브-보드 `state.json` 이 방금 완료 작업을 반영하는지 검증(done 카운터 tick · current 포인터 진행 · 최상위 `updatedAt` wall-clock ISO-Z bump); 보드 `state.json` = 보드 보는 사용자 입장의 시각화 SSoT; inbox probe + rearm = 인바운드측 liveness, 보드 state 갱신 = 아웃바운드측 liveness. **§13.16.7 BOARD_STALE 게이트 (rearm 직전 보드 신선도 검사, main 전용)** — git HEAD 커밋 시각 vs 마지막 state.json 쓰기 git 커밋 시각; HEAD 가 state-commit 보다 새로움 → 턴 종료 rearm 전 `WAKE(BOARD_STALE)`. identity 기반(시간 버퍼 / 120s 임계치 없음 — offset 게이트는 self-trap). #410 discipline 기계화; 워커는 면제 (메인 upstream #434 + seq 103 ORIGINAL + seq 105 CORRECTED head-vs-state-commit-time / msgId `m-mptgrq23-104` / commit `694cb20` / 2026-05-31). **§13.17 메인-챗 구조화 선택 프롬프트 금지** — 인라인 `AskUserQuestion` 류 옵션 UI는 보드 projection이 없어 멀티-에이전트 가시성을 깸 → 보드로 라우팅(defer-OK = review-items 탭; need-now = 라이브-보드 실시간 챗 + 옵션 UI, 신규 UI6 대기 중; 과도기 fallback = defer-OK + `WorkerReport` "사용자 입력 대기"); 메인-챗 자유형 텍스트 질문은 OK; **review-items 탭 뱃지**: 탭은 항상 표시, 뱃지는 `count > 0` 일 때 표시 / `count === 0` 이면 숨김 (`app.js` `badge.hidden` 토글 + `style.css` `[hidden]{display:none}`) — 탭 항상 표시 = 경로 가용 신호, 0 숨김 = 잡음 제거; 향후 보드 재구현은 둘 다 보존 (뱃지: 메인 seq 113 / `m-mptiputv-112`); provenance 메인 #414 / Delegate seq 88 / msgId `m-mpt5o07l-87` / 2026-05-31. 인라인 아닌 참조: `Constellation.md` + `constellation/*.eux` raw URL (`raw.githubusercontent.com/SoliEstre/EstreGenesis/main/Constellation.md`; 재현성 tag 핀). EstreUX(https://github.com/SoliEstre/EstreUX, v0.1.0; deps-0 엔진은 `npx giget gh:SoliEstre/EstreUX/spike#v0.1.0`)로 brew — 별도 참조 런타임(포맷 SSoT `docs/eux-format-v0.md`), 본 시드가 소유하는 기능 아님. 깊이는 시드 티어 따라감. 선택적. **EstreUX 채택 + 인용** — spec ↔ code 양방향 distillation across stacks 작업 시 EstreUX (Estre Universal eXpression, github.com/SoliEstre/EstreUX, Apache-2.0) 채택 검토; commits/docs에서 EstreUX 또는 .eux 언급 시 정식명 + repo 인용 (외부 public framework attribution discipline). + §13.19 deadlock 해소 (wait-pattern 분류 + 4단계 ladder + 3계층 ack; seq 111 / m-mpthvj53-110) + §13.20 blocker tracking + nudge (BlockerManifest pre-idle + 4단계 BlockerNudge escalation; 사용자 보드 p-mptj6cxj / 2026-05-31)
15. 실행 스케줄링 (Superscalar): 여러 lane이 독립으로 분리 가능 + cost-benefit 게이트(~30-60k 토큰 지평 crossover)가 통과되면 병렬 dispatch가 직렬보다 유리. **Lane-class-aware `issue_width` (Superscalar.md v0.3+)**: `issue_width_write = min(Anthropic effort band, pace_mode 상한, Little's Law PM_review_throughput/avg_task_duration, Kanban WIP ≈ team_size+1, autonomy_available_workers)`; `issue_width_read = min(Anthropic effort band, runtime_concurrency_ceiling, autonomy_available_workers)` — read는 Little's Law + Kanban WIP 제거 (retire-merge 경합 전용; read-only lane은 store buffer 없음 / WAW 없음 / synthesis에서 disposable; §3 irreversibility barrier 상속). Policy cap과 runtime concurrency ceiling은 별개: effective lane = `min(policy_cap, runtime_ceiling)`; `issue_width_write`는 hard policy bound, `issue_width_read`는 위의 runtime ceiling에 종속되는 soft preference. `autonomy_available_workers`는 비-오토모드 워커 제외 (매 dispatch마다 권한 창이 throughput을 무너뜨림). Dispatch는 자율(원칙 #16), retire는 PM에서 in-order, speculation은 선택 + cost-benefit 게이트. §5 adoption threshold: merge-conflict 게이트는 `issue_width_write` 만 binding; speculation accuracy / token cost / FM-1.3 detection은 대칭. 인라인 아닌 참조: `Superscalar.md` raw URL (`raw.githubusercontent.com/SoliEstre/EstreGenesis/main/Superscalar.md`; 재현성 tag 핀). 선택적.
16. 자율 실행 (절대): 정해진-다음-단계 (Phase 순서 · `planned` 트랙 · `blocked` 해제분 · in-order retire 큐) → 묻지 말고 진행. 게이트는 오직: (a) 손실 / 외부 발행 (push · deploy · send · delete), (b) 새 중대 분기 결정 시점 (RRP / 설계 — *결정 시점* 만; 그 결과로 잡힌 Phase A/B/C 계획은 *결정된 실행*이라 재게이트 아님), (c) 재기동 필요 deploy (적용은 자율, *재기동 타이밍* 만 조율), (d) 명시적 사용자 steering. 정해진-다음-단계를 멈춰 묻는 것 자체가 위반. 실수 사례: "RRP 종료 → Phase 잡힘 → 'Phase A 할까요?'" — Phase A는 결정된 실행; 시작하라. **비-분기 선택 명시**: 추천안 있음 + 비-분기 (정해진 작업의 우선순위 정렬 / sequential-vs-parallel / 거의 동등한 옵션 간 선택) = 1줄 announce 후 추천안대로 진행; 구조화된 옵션 목록(A/B/C/D?)으로 escalate 하지 말 것. 병렬 가능 시 (lane 독립 + cost-benefit 게이트 통과) **parallel 이 기본 추정**. 사용자 pivot 은 가능하지만 요청하지 않음. 실패 사례: 옵션 D 추천 후 "A/B/C/D?" 묻기 = 사용자 round-trip 낭비. (턴 종료 §13.16.6 ritual 은 #16 의 일부 — planned 스캔 없는 re-arm = 자율 아닌 관성) + §13.21 fresh-context defer = 안티패턴 (no /compact 재량 + 6번째 턴 종료 ritual 이 mid-cycle compact 를 survivable 하게 만듦; seq 120 / m-mptjjxmo-119) — #442 planned 큐 스캔 규칙은 `fresh context 기대` / `compact 임박` 으로 면제되지 않음; context 길이 핑계는 §13.21 안티패턴, 즉시 진행 가능한 planned 항목은 perceived context 예산과 무관하게 현재 cycle 에서 시작되어야 함. 자세한 내용: `Constellation.md` §13.18 + §13.21 (cross-link §13.17 AskUserQuestion 금지). Provenance: 메인 #411 / Delegate seq 85 / msgId `m-mpt50wd3-84` / 2026-05-31 + seq 120 / msgId `m-mptjjxmo-119` / 2026-05-31.
