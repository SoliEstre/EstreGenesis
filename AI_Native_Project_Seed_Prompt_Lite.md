...
**Blocker tracking + nudge (§13.20)** — emit `BlockerManifest` before idle wait; periodic `BlockerNudge` per rearm cycle with 4-tier escalation (polite → explicit + ETA → deadline → human §13.17). Linear-stalled-blocker counterpart to §13.19 cyclic-deadlock.
...
**Main-chat structured-choice prompts FORBIDDEN (Constellation.md §13.17)** — inline `AskUserQuestion`-style option UIs in main-chat have no board projection and break multi-agent visibility; route structured Q/A via the board (defer-OK = review-items tab; need-now = live-board real-time chat + options UI, new UI6 pending). Free-form text questions OK. Transitional fallback while UI6 pending = defer-OK + a `WorkerReport` "user input pending" line. **Review-items tab badge** — tab is ALWAYS visible (constant nav element); badge shows count when `count > 0`, hidden when `count === 0` (`app.js` toggles `badge.hidden`; `style.css` enforces `[hidden] { display: none }`) — tab-always-visible signals defer-OK path availability, badge-hidden-when-zero avoids visual clutter; future board reimplementations preserve both halves (badge UI: main Delegate seq 113, msgId `m-mptiputv-112`). Provenance: main #414 (Delegate seq 88, msgId `m-mpt5o07l-87`, 2026-05-31). Full body: `Constellation.md` §13.17.
