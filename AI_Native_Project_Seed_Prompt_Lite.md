...
<!-- seed-tier: Lite; language: English; version: v2.5.0; date: 2026-06-11; counterpart: AI_Native_프로젝트_시드_프롬프트_Lite.md; changelog: upstream EstreGenesis repository CHANGELOG.md, not target project README.md; v2.5.0 = pruning+registry cut — §13.x operating disciplines pointerized from inline to the Constellation.md SSoT (seed keeps invariant vocabulary + one-line gists) + provenance/verbatim redaction + § EG Module Registry (one-table of 5 modules) added -->
...
**Operating disciplines — pointers, not inline (Constellation.md §13.x)** — the full-text SSoT is `Constellation.md` §13.x; the seed keeps only the invariant vocabulary + a one-line gist. **§13.11 board emission**: progress emits mandatory at every safe point · no autonomous heartbeat during idle (false-alive). **§13.13 A2A ack layer**: three delivery grades `delivered`/`read`/`processed` (ROGER ≠ WILCO) · vocabulary `msgId` (dedup watermark) / `Ack{delivered}` (automatic; acks are never acked) / `AckProcessed` / `AckCumulative` / `Ping/Pong` (liveness probe — not a retransmission tool) · on ack timeout: conservative Ping → check own inbox dedup → retransmit only what was lost → escalate to a human (Two Generals closure) · server/bridge/agent layer separation. **§13.16.6 turn-end ritual (6 elements, fixed order)**: ① inbox cursor-tail probe (mandatory before rearm) ② unconditional watcher rearm (launched ≠ live) ③ live-board state refresh (done tick · advance current · bump `updatedAt`) ④ BOARD_STALE gate (§13.16.7, HEAD vs state-commit identity comparison, main-only) ⑤ planned-queue scan (distinguish blocked/planned; start startable items now) ⑥ context materialization before large work (durable artifact → post-compact pickup). **§13.17 main-chat structured-choice prompts FORBIDDEN**: inline option UIs have no board projection → defer-OK = review-items tab / need-now = live-board chat + option UI; free-form text questions OK; tab always visible + badge only when count>0. **§13.19 deadlock resolution**: wait-pattern strict/quasi/healthy classification + 4 layers prevention/detection/auto-resolution/escalation + 3-tier ack (transport → commitment → application). **§13.20 blocker tracking + nudge**: emit `BlockerManifest` before idle + periodic `BlockerNudge` with 4-tier escalation — the linear-stalled counterpart to §13.19. **§13.21 fresh-context defer anti-pattern**: never defer planned work to a next session; ⑥ materialization makes mid-cycle compaction survivable.
