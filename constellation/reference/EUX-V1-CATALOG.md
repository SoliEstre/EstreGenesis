...
- **`@render` N/A or `@targets vanilla` misuse instances**:
  ...
  - **Clarification (P2 §3.10 sub-spec)**: the missing `@render` on these 5 UI files is **intentional and v1-conformant**. The UI profile makes `@render` **optional** when the UI is template-rendered upstream by a parent component (the component declares structure via `@ports.cmd`/`@state` and the parent owns the DOM template); explicit `@render` is only required when the component owns its own render shape. Only `@render N/A` placeholder or `@render` on backend/protocol profile is **forbidden** per §3.10 + §4 G3 — bare omission on a UI component that is upstream-template-rendered is permitted.
...
6. **Machine integrity**:
   ...
   - **Initial-state convention (P3 resolution)**: when no `(initial)` annotation is present, drift-check applies the convention **"first state in the list is implicit-initial"** (no WARN if this convention is acceptable). An optional explicit `initial: <state>` annotation may override the implicit-first convention. See eux-format-v1.md §3.7 for the full rule and multi-axis form.
...
3. **`@behavior` clause status**: server.eux ships a ~65-line `@behavior` block that is neither `@machine` (it's prose-with-bullets) nor any of the v1 four-directive set (P1 sub-slice: `@ports` / `@machine` / `@source` / `@deps`-absorbed; see eux-format-v1.md §2 canonical phrasing — the full v1 set is 9 directives). Does v1 (a) formalize `@behavior` as a fifth directive, (b) require migration into `@machine.derive` / `@machine.guard`, or (c) tolerate it as out-of-band documentation that drift-check ignores? Real risk: server.eux is the most critical orchestration file in the repo and a forced migration could lose incident context. **(Resolved per seq 73 → answer (a) `@behavior` formalized as 5th formal directive, doc-tier; see eux-format-v1.md §3.9 + Appendix A Q3.)**
...
## Verify (runtime 6-check matrix — 2026-05-31)

> **Label note (collision fix — main upstream Delegate seq 94, msgId `m-mptc7s1v-93`)**: These 6 verification checks are **operational drift-detection runs against the runtime `.cjs` implementations**, NOT the same as the **structural 6 gates** defined in `constellation/reference/docs/eux-format-v1.md` §4 (Drift-Check 6-Gate Matrix). The structural gates (spec §4: G1 ports / G2 machine / G3 deprecated render-persist / G4 standalone-deps / G5 targets-placeholder / G6 source-provenance) are the **spec contract** — they describe what a `.eux` file must look like to be v1-conformant. The 6 runtime checks below (G1 ports / G2 machine / G3 coverage / G4 derive-anchor / G5 envelope / G6 return-shape) are **observational validation** that the contract holds against the actual `.cjs` runtime implementations on disk today. Same gate count, different gate sets; the spec §4 matrix is the SSoT contract, this `Verify` section is implementation-side evidence. Use the label **"Verify 6-checks"** (or **"runtime 6-check"**) when referring to this matrix to avoid confusion with the spec §4 "6-gate" contract.

**Summary**: 12 files surveyed against v1 spec — **4 PASS · 6 WARN · 2 FAIL** (2 CI blockers from spec §4 G5 STRICT FAIL — surfaced by main upstream Delegate seq 94 / msgId `m-mptc7s1v-93`). The 2 FAIL files are **watchdog.eux** (backend) and **ws-core.eux** (protocol) — both shipped `@targets vanilla`, forbidden outside ui profile per spec §4 G5 / §3.4 / §6.1 migration table. The prior "0 FAIL" summary contradicted §4 G5 STRICT: the §4 G5 contract gate STRICTLY fails any non-ui file with a placeholder `@targets` (vanilla / estreuv / TBD / none / N/A), and `vanilla` on backend (`watchdog.eux`) + protocol (`ws-core.eux`) is exactly the forbidden case. This commit corrects the count and substitutes `@targets vanilla` → `@targets runtime` on both files per spec §3.4 ("For backend / protocol profile, use `runtime`") + §6.1 migration table (`@targets vanilla` on backend/protocol → "Change to `@targets runtime` or a specific stack name"). The runtime C1..C6 matrix below does not directly observe G5 (there is no `@targets-placeholder` column among C1..C6); the 2 FAIL classifications are sourced from the spec §4 structural gate matrix and overlaid onto the **Overall** column of those two rows below for honest accounting (watchdog.eux: prior C-row Overall WARN → now **FAIL** because §4 G5 STRICT overrides; ws-core.eux: prior C-row Overall PASS → now **FAIL** for the same reason). Remaining WARN-tier issues cluster around (a) `@source` adoption gap (0/12, expected — WARN-tier in v1), (b) `cmd`-under-`in` label mislabeling on three UI files (auto-fixable per §3.6 rule 1), (c) ambiguous empty `out`/`deps` without explicit `# none` comment on `ws-conn-bar`, (d) non-standard `@machine` dispatch format on `ws-tool-card`. The 6 runtime checks as evaluated: (C1) per-profile required `@ports` sub-clauses; (C2) `@machine` states+initial-marker (STRICT for protocol, soft-WARN for backend, OPTIONAL for ui); (C3) every `states` enum value reachable via `dispatch` + entry-actions traceable; (C4) `@derive` clauses cite a code anchor or named incident (WARN tier); (C5) `@envelope` raw literal scan vs matching `.cjs` (server.cjs · ws-core.cjs · watchdog.cjs · local-bridge.cjs); (C6) `@ports` `cmd` return-shape parity vs implementation (verified for ws-core `decodeFrame`/`handleUpgrade` against `ws-core.cjs` lines 34/51/117-122). **Note**: the C-prefix (C1..C6) used in this matrix deliberately mirrors but does not equal the spec §4 G-prefix (G1..G6); a row passing C2 here means the runtime observation matched the spec, whereas G2 in spec §4 is the contract definition itself. The 2 FAIL count is sourced from spec §4 G5 (contract-level), not from C1..C6 (runtime-observation level).

| File | Profile | C1 ports | C2 machine | C3 coverage | C4 derive | C5 envelope | C6 return | Overall |
|------|---------|----------|------------|-------------|-----------|-------------|-----------|---------|
...
| watchdog.eux | backend | PASS | PASS | PASS | WARN | PASS | WARN | **FAIL** (§4 G5) |
...
| ws-core.eux | protocol | PASS | PASS | PASS | PASS | PASS | PASS | **FAIL** (§4 G5) |
...
**Counts**: PASS=4 · WARN=6 · FAIL=2 · TOTAL=12. (Prior count was PASS=5 · WARN=7 · FAIL=0 — corrected per main upstream Delegate seq 94 / msgId `m-mptc7s1v-93` which surfaced the G5 STRICT contradiction.)

**Blockers** (FAIL-tier — would block CI): **2 blockers** — both spec §4 G5 STRICT FAIL (`@targets` placeholder on non-ui profile):
- **watchdog.eux** (backend) — shipped `@targets vanilla`; resolved in this commit by substituting `@targets runtime` per spec §3.4 + §6.1 migration table.
- **ws-core.eux** (protocol) — shipped `@targets vanilla`; resolved in this commit by substituting `@targets runtime` per spec §3.4 + §6.1 migration table.

The other STRICT checks remain clean (C1 universally satisfied; C2 STRICT satisfied for all 3 protocol files; no `@deps` standalone per §4 G4; no forbidden `@render`/`@persist` per §4 G3). Once the `@targets vanilla` → `@targets runtime` substitution is committed, the next `verify` run should restore the count to PASS=6 · WARN=6 · FAIL=0 (watchdog promotes from FAIL to WARN per pre-§4-G5 row-level Overall; ws-core promotes from FAIL to PASS per pre-§4-G5 row-level Overall).

**WARN summary** (recommended fixes, ordered by impact):
- **C4 derive anchors** (3 files: gateway-client, local-bridge, watchdog) — promote inline `@intent` provenance notes to explicit `@source` or anchor refs; not blocking but improves drift-check fidelity.
- **C6 cmd-label drift** (3 UI files: ws-channel-input, ws-collab-invite, ws-conn-bar) — `cmd` lines listed under `in:` label; auto-fix per §3.6 rule 1 (move to dedicated `cmd:` sub-clause).
- **C6 watchdog cmd/out mixing** — `restartServer`/`restartBridge` declared as `out:` but are internal-spawned commands; clarify per v1 sub-clause semantics in §3.6.
- **C3 ws-tool-card dispatch format** — non-standard `phase→action` notation rather than formal `state + event → state'`; reformulate per §3.7.
- **C4/C5 N/A rows** — runtime adapters (gateway-client agent-side, UI components) have no matching `.cjs` envelope to scan; these are vacuous-PASS but recorded as N/A to keep the matrix honest.

**Spec cross-reference**: structural-gate definitions per `constellation/reference/docs/eux-format-v1.md` §4 (the **spec 6-gate matrix** — G1..G6, contract-level) + §3.6 (`@ports` validation rules) + §3.7 (`@machine` STRICT tier dispatch). The C1..C6 runtime checks above are observational counterparts; they do not redefine the spec gates, they only verify that current `.cjs` implementations behave consistently with the `.eux` contracts the spec defines.
...
- **Drift-check verification**: runtime 6-check Verify matrix run 2026-05-31 (see §Verify above; renamed from "6-gate" to "6-check" per main upstream Delegate seq 94 / msgId `m-mptc7s1v-93` to disambiguate from the spec §4 structural 6-gate contract matrix) — **4 PASS · 6 WARN · 2 FAIL** (FAIL count corrected from prior "0 FAIL" per seq 94 G5 STRICT recheck; 2 blockers are `watchdog.eux` + `ws-core.eux` `@targets vanilla` on non-ui — resolved in this commit by substituting `@targets runtime`).
- **P2/P3 follow-up sync**: Delegate seq 94 (`m-mptc7s1v-93`) follow-up — addressed P2 items (cmd-clause vs G6 parity clarification; directive-count canonical phrasing; `@render` 5/6 intentional-omission note) and P3 items (G2 initial-state convention with `initial:` annotation override; mixed per-half fallback rule). Spec edits land in eux-format-v1.md §2 / §3.6 / §3.7 / §3.10 / §6.2 / §7.5; catalog edits land in the §Drift summary `@render` bullet + the §Validation rules machine-integrity entry + the §Open questions Q3 cross-reference. Additions only; existing content preserved.
