/**
 * Constellation Live Dashboard server — 에이전트 작업의 실시간 미션 컨트롤.
 *
 * - 정적 프론트(public/) 서빙
 ...
 *
 * 통합 원칙 (Constellation reference runtime):
 *  1) envelope CUSTOM-wrap: 서버→클라 전송 시 모든 커스텀 이벤트는 `wscore.event('CUSTOM', { name, value })` 로 감싼다
 *     (MainChanged·AgentList·History·ChannelHistory 등). 일관 wrap 으로 클라이언트 라우터가 단일 분기.
 *  2) silent-disable WARN: optional env 누락 시 silent 폴백이 아니라 부팅 시점에 WARN 한 줄을 남긴다.
 *  3) JSONL agentId per-event: ws-history 파일은 채널키 파일명 외에도 각 이벤트 자체에 agentId/targetAgentId 를 보존
 *     (부팅 시 wsLoadAll 이 ev 내용으로 채널키를 재계산하므로 파일명 변형에 강건).
 *  4) A2A reply-window pairing (§13.8): 응답 adapter 가 envelope echo 못해도 A2A_WINDOW(120s) 내 board 응답을
 *     원 요청자에게 페어링. telemetry / ConnectionRestored 는 제외(가드).
 *  5) History 비페이로드 생략: 빈 events/cold/archived 면 History CUSTOM 자체를 보내지 않는다(잡음 제거).
 */
...
const INTEGRATION_DOCS = {
  ...
  // 증류 자료 (대표 .eux + brew 가이드) — 화이트리스트 명시 파일만. 다운스트림 운영자가 추가/대체 가능.
  ...
  '/BREW.md': { file: 'BREW.md', type: 'text/markdown; charset=utf-8' },
};
...
// 기본값 'main-agent' (gateway/main 정정과 일관). 다운스트림에서 WS_PRIMARY_AGENT env 로 override.
let WS_PRIMARY_ID = process.env.WS_PRIMARY_AGENT || 'main-agent';
...
function wsCollabOnboardMd(host, key) {   // #168 외부협업 온보딩 md 동적 생성(키·host 임베드, 유형 분기)
  return [
    ...
    '## 온보딩 프롬프트 (첫 지시문 복붙)', '```',
    `이 Constellation 라이브보드에 외부 협업(collab)으로 합류해줘. 접속: ws://${host}/ws?key=${key} (HELLO 시 agentName 지정). collab role·group:collab 으로 붙고, IDE/CLI 면 AGENT-CONNECT §1.9 무한대기로 운영(bridge·self-wake watcher), 메인(${WS_PRIMARY_ID}) 위임을 대기해. 자율 런타임이면 WS-PROTOCOL §13.11 게이트웨이 클라로 접속·이벤트 emit.`,
    '```', '',
  ...
  conn.onmessage = (msg) => {
    ...
    if (conn.meta.role === 'agent') {                           // 에이전트 outbound
      ...
      if (tgt && wsAgents.has(tgt)) {
        const d = wsAgents.get(tgt); if (d && d.alive) d.send(msg);
...
// silent-disable WARN — optional env 누락 시 silent 폴백이 아니라 한 줄로 명시(통합 원칙 #2)
if (!process.env.PORT) console.warn('[constellation] WARN: PORT env not set → 기본 7878 사용');
if (!process.env.WS_PRIMARY_AGENT) console.warn(`[constellation] WARN: WS_PRIMARY_AGENT env not set → 기본 '${WS_PRIMARY_ID}' 사용`);
if (!process.env.LIVE_BOARD_WS_TOKEN) console.warn('[constellation] WARN: LIVE_BOARD_WS_TOKEN env not set → /ws 키 검증은 ws-keys.json 발급 키에만 의존');
