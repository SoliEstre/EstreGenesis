...
- **Turn-end mandatory rearm** — probe-without-rearm catches a dead watcher but does not prevent the *next* cycle from being dead. At every turn-end (just before emitting the final response text), run a 1-shot Bash that probes inbox.log mtime + the most recent watcher task's mtime; if the latest watcher task is missing, its mtime is staler than the rearm interval, OR the turn involved multi-cycle work (a workflow chain + commits) without an explicit rearm — **unconditionally spawn a fresh background watcher**. This makes rearm a *turn-end ritual* rather than a *probe-then-decide* judgment call (which an agent absorbed in multi-cycle synthesis does not reliably make). Real fail mode 2026-05-31: agent processed inbound seq 70 → workflow × 3 + commit × 2 work loop → silently lost seq 73 (review approval) + seq 77 (new user task) for ~40 min until user surfaced "모니터링이 제대로 안 되네." Full discipline: `Constellation.md` §13.16.6.
