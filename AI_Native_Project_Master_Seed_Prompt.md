...
<!-- seed-tier: Master; language: English; version: v2.5.0; date: 2026-06-11; counterpart: AI_Native_프로젝트_마스터_시드_프롬프트.md; changelog: upstream EstreGenesis repository CHANGELOG.md, not target project README.md; v2.5.0 = pruning+registry cut — §13.x operating disciplines pointerized from inline to the Constellation.md SSoT (seed keeps invariant vocabulary + one-line gists) + provenance/verbatim redaction + § EG Module Registry (one-table of 5 modules) added -->
...
| --- | --- | --- | --- | --- |
| Constellation | Real-time multi-agent live board + A2A messaging | Concurrent multi-agent operation needs real-time visibility · live A2A · delegation orchestration (file-based coordination no longer suffices) | `Constellation.md` | `plugins/constellation` |
| Superscalar | Parallel-dispatch scheduling policy (issue_width + in-order retire) | pace_mode burst/sprint + work splits into independent lanes + cost-benefit gate (~30-60k token horizon) clears | `Superscalar.md` | `plugins/superscalar` |
...
### Operating disciplines — pointers, not inline (Constellation.md §13.x)

The full-text SSoT for Constellation operating disciplines is `Constellation.md` §13.x (raw URL on `main` for latest; pin a tag for reproducibility). The seed keeps only the **invariant vocabulary + a one-line gist** per discipline — adopting agents read the full body at the pointer:

- **§13.11 Board emission discipline** — progress emits are mandatory at every safe point (§13.11.1; the board alone must suffice to reconstruct session flow); no autonomous heartbeat during idle (§13.11.2; watchdog heartbeats are false-alive — connection liveness ≠ turn liveness).
- **§13.13 A2A ack layer** — transport is in-order best-effort; reliability lives in the protocol layer. Three delivery grades: `delivered` (server) · `read` (bridge cursor) · `processed` (agent WILCO); ROGER ≠ WILCO. Vocabulary: `msgId` (dedup watermark) · `Ack{kind:'delivered'}` (automatic; acks are never acked) · `AckProcessed` · `AckCumulative` · `Ping/Pong` (liveness probe — **not a retransmission tool**). On ack timeout: conservative Ping → check own inbox dedup → retransmit only what was lost → if still silent, report to a human (Two Generals closure). Layer separation: server/bridge/agent; the server never auto-pongs.
- **§13.16.6 Turn-end ritual (6 elements)** — a launched watcher ≠ a live watcher (silent-stall). Fixed order just before every turn close: ① **inbox cursor-tail probe** (mandatory before rearm — tail from the last surfaced cursor → surface meaningful inbound → advance cursor) ② **unconditional watcher rearm** (regardless of apparent liveness; false-positive spawn cost < false-negative miss cost) ③ **live-board state refresh** (tick done counters · advance the current pointer · bump `updatedAt` ISO-Z — the board is the user-facing visualization SSoT) ④ **BOARD_STALE gate** (§13.16.7; identity comparison of git HEAD commit time vs last state.json commit time, no time buffer; main-only, workers exempt) ⑤ **planned-queue scan** (distinguish `blocked` (external dependency) from `planned` (autonomously startable) — start startable items now; idle is reserved for true exhaustion) ⑥ **context materialization before large work** (record scope/files/anchors in a durable artifact → post-compact pickup; in-memory plans do not survive compaction). The key is mechanizing this as a *turn-end ritual*, not a probe-and-decide judgment.
- **§13.17 Main-chat structured-choice prompts are FORBIDDEN** — inline `AskUserQuestion`-style option UIs have no board projection and break the board-as-SSoT contract. Free-form text questions are fine. Routing: defer-OK → board review-items tab (default) / need-now → live-board realtime chat + option UI. The review-items tab is always visible; the badge shows only when `count > 0`.
- **§13.19 Deadlock resolution** — wait-pattern classification strict (true cycle) / quasi (resolvable by deferred work) / healthy (bounded) + four layers: prevention / detection / auto-resolution / escalation (ordering → preemption → mediation → human via §13.17). Three-tier ack above §13.13: transport → commitment (`ReviewSLAAck`) → application.
- **§13.20 Blocker tracking + nudge** — when the planned queue is all-blocked, emit a `BlockerManifest` *before* entering idle + periodic `BlockerNudge` with a 4-tier escalation (polite → explicit + ETA → deadline → human). The linear-but-stalled counterpart to §13.19's cyclic deadlock.
- **§13.21 Fresh-context defer is an anti-pattern** — never defer planned work to a hypothetical fresh context / next session. Compaction fires outside agent control; §13.16.6 ⑥ materialization is the mechanism that makes mid-cycle compaction survivable — cycles get resumed, not deferred.
