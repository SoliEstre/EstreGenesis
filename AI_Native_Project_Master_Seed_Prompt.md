...
<!-- seed-tier: Master; language: English; version: v2.4.3; date: 2026-06-03; counterpart: AI_Native_프로젝트_마스터_시드_프롬프트.md; changelog: upstream EstreGenesis repository CHANGELOG.md, not target project README.md; v2.4.3 = main hub feedback reflection — Migration B delta table catch-up: previously-missing rows for #12 Constellation (v2.0.0) and #13 Superscalar + #14 autonomous-execution (v2.3.0). v2.4.2 added the dual-track abstract guidance; v2.4.3 adds the concrete delta rows the guidance points at. Itself a release-only cut surfaced by main's actual hub migration to v2.4.2. -->
...
- **Turn-end mandatory rearm** — at every turn-end (just before emitting the final response text), **UNCONDITIONALLY spawn a fresh background watcher** — do NOT skip on apparent active-liveness (false-positive spawn cost < false-negative miss cost). **Cursor-tail probe MUST run BEFORE rearm (mandatory ordering)**: (a) tail inbox.log from last-surfaced cursor → (b) surface any meaningful inbound → (c) advance cursor → (d) THEN unconditional rearm. The probe is the PRIMARY catch for the armed-window blind spot (NOT diagnostic telemetry); rearm is forward-looking. Skipping the probe loses inbound from the prior watcher's SIGSTOP-suspend window. This makes rearm a *turn-end ritual* rather than a *probe-then-decide* judgment call (which an agent absorbed in multi-cycle synthesis does not reliably make). Real fail mode 2026-05-31 (a): agent processed inbound seq 70 → workflow × 3 + commit × 2 work loop → silently lost seq 73 (review approval) + seq 77 (new user task) for ~40 min until user surfaced "모니터링이 제대로 안 되네." Real fail mode 2026-05-31 (b): agent rearmed without first probing → seq 123/127 inbound from prior-suspend window lost; user surfaced "메인이 회신 대기중이라는데". Full discipline: `Constellation.md` §13.16.6.
