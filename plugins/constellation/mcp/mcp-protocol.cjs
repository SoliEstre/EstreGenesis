...
// SOURCE OF TRUTH — do not edit the copies.
//
// A plugin is installed as a copy of its own directory and nothing else, so a server that reaches
// `../../_shared/` from inside `plugins/<name>/mcp/` finds nothing there once installed: `node`
// exits before the first JSON-RPC line and every client reports "connection closed: initialize
// response". This file is therefore copied verbatim next to each server that needs it
// (`plugins/<name>/mcp/mcp-protocol.cjs`) by `node scripts/sync-shared-parts.mjs --write`, and the
// maintenance checker fails the build when any copy differs from this file. Edit here, then sync.
