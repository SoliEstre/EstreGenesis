...
- **The aliases are pointers, not copies.** `/egboot`, `/egmig` and `/egup` each read their canonical sibling and execute it — [Compendium](../../Compendium.md)'s pointer-not-paraphrase discipline applied to this plugin's own surface. Two names, one procedure, zero drift.
