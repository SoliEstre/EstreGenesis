...
| `active-profile (v0.7.0, MUST-20)` | At IR-emit time, ALSO write `.hyperbrief/active-profile.json` = `{ decision_id, emitted_at: <ISO now>, declared: { audience, abbreviation, jargon } }` (the tone profile you just declared). This arms the post-response tone-estimate Stop hook (§5.6.7 post-response tier — deterministic declared-vs-effective drift advisory, zero-cost when the file is absent). Delete the file when the decision cycle closes (resolved/archived); the hook also TTL-advises after 24h. |
