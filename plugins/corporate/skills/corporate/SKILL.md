...
- **`lane` is required** (§9.1): `interactive` (subscription sign-in, a person working with the seat at ordinary individual intensity) or `automation` (metered key — unattended residency, scheduled seats, and any fan-out). Do not assume a credential permits more than its lane; where a seat's intensity exceeds ordinary individual use, it belongs in the automation lane by design.
