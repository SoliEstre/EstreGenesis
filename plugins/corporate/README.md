...
Durable-role agent organization. Wraps [`Corporate.md`](../../Corporate.md) v0.1.1.
...
One entry is a **status warning rather than a row to use**: Gemini CLI stopped serving consumer-plan (Google AI Pro / Ultra / free) requests as of 2026-06-18, so a seat backed by a consumer Google subscription belongs on `agy` instead. A seat whose harness is unresolved can still be **declared but not launched** — the honest failure is a refusal, not a guessed flag.
