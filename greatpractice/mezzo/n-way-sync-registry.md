...
source_evidence:
  ...
  - "EG-side own N=1 MISS (2026-06-07, v2.5.67 에서 발견): v2.5.62 Hyperbrief v0.6.0 cut 의 CHANGELOG 가 '.claude-plugin + renderers + mcp 3 package.json 동시 bump' 라 적었으나 실제론 plugin.json + renderers/package.json 가 0.5.6 에 stale (mcp/marketplace/Hyperbrief.md 만 0.6.0). 본 mezzo 의 atom #1 Check(stale grep 0건)가 *수행되지 않아* 드리프트가 한 cut 동안 굳음 — 즉 practice 부재가 아니라 enforcement 부재. f7b2a7a 로 정정 + EG-ops 결정론적 verify 스크립트로 §5 enforcement gap 메움."
...
revision_history:
  ...
  - {ts: 2026-06-08T00:00:00Z, type: evidence+enforcement, by: claude-opus-4-8, cost_tier: null, note: "v2.5.67 — own N=1 MISS 기록 (v2.5.62 Hyperbrief plugin.json+renderers 0.5.6 stale, atom #1 Check 미수행). §5 enforcement gap 을 EG-ops 결정론적 verify 스크립트로 메움 (axis별 표면 버전 read → 일치 assert, post-cut 실행). miss_count.compulsory 0→1."}
...
rrpv: 2
miss_count:
  compulsory: 1   # 2026-06-07 v2.5.62 Hyperbrief plugin.json+renderers 0.5.6 stale (atom #1 Check 미수행 → 드리프트). v2.5.67 f7b2a7a 정정 + deterministic verify 도입.
  capacity: 0
  conflict: 0
  coherence: 0
...
> **§5.0 결정론적 verify — enforcement gap 의 첫 실현 (2026-06-07/v2.5.67)**: 본 mezzo 가 v2.5.61 ratify 후에도 §1 atom #1 의 Check("stale grep 0건")가 *수행 의존*(수동·skippable)이라, v2.5.62 Hyperbrief cut 에서 plugin.json+renderers 가 0.5.6 에 stale 한 드리프트가 한 cut 동안 굳음 (own N=1 MISS). 교훈: **practice 존재 ≠ enforcement**. 해결로, atom #1 Check 를 **결정론적 스크립트**로 기계화 — axis(같은 버전을 공유해야 하는 표면 집합)별로 각 표면의 버전 문자열을 read → 일치 assert → 불일치 시 목록+exit 1. release cut 직후 실행해 비결정성 0 으로 드리프트를 잡는다. **핵심 설계 교훈**: axis 정의의 정밀도가 verify 정확도를 좌우 — 모듈마다 "동조해야 하는 표면"이 다르다 (예: Hyperbrief 는 plugin/renderers/mcp/marketplace/spec 가 단일 axis 0.6.0 이지만, Constellation 은 plugin 버전 0.3.x 과 MCP 서버 버전이 *별개 axis* — mcp 를 잘못 묶으면 false-positive). 즉 §6 surface 등록부가 axis 단위로 정확해야 함. EG 는 본 verify 를 운영측 스크립트로 두고(프로젝트-특수 표면 경로 포함), 본 mezzo 는 그 *패턴*을 공개 규율로 codify.
