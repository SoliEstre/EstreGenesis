...
> The `routinize` skill (`plugins/greatpractice/skills/routinize/`) is the model-invoked front door to stages 1–3 (capture → mature → codify draft); stages 4–5 (enforce / revisit) stay user-gated. Its prime trigger is `/before-compact` time — the session's accumulated context is the raw material and is about to be compacted away, so cross-session durable practices are captured before session-specific resume state.
