...
This section is an abridgment of the modality backfill. Once the v0.2 cycle's full research is complete, it will be promoted to a separate §normative-foundations.distributed-cognition section.

**Core theses**:
- Hutchins (1995, *Cognition in the Wild*) ship-cockpit / aircraft-cockpit analysis: cognition is distributed across *system + artifact + person*, not *inside an individual's head*. A navigation *fix* is not the cognition of a single navigator but the *collective cognitive product* of chart + instrument + crew.
- Clark-Chalmers (1998, *The Extended Mind*): for a notebook to be recognized as part of the mind, the 3 *coupling conditions* must be met — (a) **reliable** (consistently accessible when needed), (b) **accessible** (search / recall is intuitive), (c) **automatically endorsed** (the content is trusted without doubting it each time).

**EG application**:
- The normative ground for *why these fields* in §3.2 frontmatter = meeting the 3 coupling conditions. `last_validated_at` + `freshness_until` = **reliable** (stale auto-dim → reliability guaranteed). `tier` + `paths:` + `surfaces:` = **accessible** (working-set auto-load → zero recall cost). `enforcement_level` + `evidence-quality` + `recommendation-strength` = **automatically endorsed** (accompanied by validated metadata → the agent does not doubt it each time).
- The 9-axis cross-axis synthesis (Cluster C: frontmatter SSoT 9/9 universal convergence) is the *empirical* justification. Hutchins + Clark-Chalmers are the *normative* justification — the first-principle basis for *why* the extended mind's coupling conditions are a required axis of the frontmatter.
- On entering multi-agent residency (Phase 2.5): Hutchins's distributed cognition is the normative justification for Cluster H (cross-surface SSoT propagation). If agent + tool + workspace are a single cognitive system, then surface distribution is the *system's architecture*, not an *implementation detail*.

This section fills the v0.1 placeholder for the normative gap.

---

### §B.6 Normative Justification 2 — Organizational Anthropology + Institutional Isomorphism (Douglas · Powell-DiMaggio)

This section is an abridgment of the modality backfill. Once the v0.2 cycle's full research is complete, it will be promoted to a separate §normative-foundations.institutional-isomorphism section.

**Core theses**:
- Douglas (1986, *How Institutions Think*): an institution itself possesses a *classification system* and *memory*. Individuals *outsource* their thinking to the institution's categories — codification is a mechanism of *institutional thinking*.
- The 3 isomorphism mechanisms of Powell-DiMaggio (1991, *The New Institutionalism*):
  - **Coercive** isomorphism: external compulsion (law / regulation / hook block).
  - **Mimetic** isomorphism: imitation under uncertainty (cloning a successful model / borrowing a seed prompt).
  - **Normative** isomorphism: professional norms (education / professional society / telos).

**EG application**:
- The §4.2 `enforcement_level: mandatory | recommended | advisory` 3-tier corresponds 1:1 to the 3 isomorphism mechanisms:
  - `mandatory` = **coercive** (hook block / exit 2 block).
  - `recommended` = **mimetic** (additionalContext inject / suggested practice).
  - `advisory` = **normative** (frontmatter only + reference context — soft codification close to professional norms).
- This 1:1 correspondence is the normative justification for *why the classification of codification force is exactly 3 kinds and needs no more* — Powell-DiMaggio's 50 years of accumulated case studies prove that *no isomorphism pressure exists outside the 3 mechanisms*.

**Risk thesis**:
- The *coercive dominance* risk of Cluster B (deterministic hook) — Powell-DiMaggio's quantitative evidence (hundreds of case studies) that isomorphism pressure *inhibits innovation* + *path-dependency lock-in*.
- That is, once the `enforcement_level: mandatory` ratio exceeds a certain upper bound (roughly ≤ 20% of entries), institutional drift occurs — agent autonomy decay + inhibition of new pattern emergence.
- The over-engineering-avoidance note of sre §4.4 handles this risk at the *symptom level*. Organizational anthropology is the *root-cause-level* justification.

**Application decisions**:
- Adopt the §4.2 enforcement_level 3-tier — 1:1 with Powell-DiMaggio's 3 mechanisms.
- The quantitative cap of the §4.5 hook frequency budget — recommend mandatory ratio ≤ 20% (this figure will be refined after the v0.2 cycle's full research).
- The *phronesis-codify-boundary* of the §5.3 maturation gate (humanities §1.12) and this institutional-drift avoidance synthesize — making explicit the domain that *should not* be codified is the heart of the normative justification.

This section fills the v0.1 normative ground of the classification.
...
- **§B.5-§B.6** = the *abridged placeholder* of the P0b modality backfill — to be promoted to a separate section once the v0.2 cycle's full research (extracting 9 patterns each from the distributed-cognition axis + organizational-anthropology axis) is complete.
- **v0.2 entry conditions**: (a) 6-12 cycles of dogfood accrual on this v0.1 spec, (b) completion of the P0a patch (6 EG-specific lapses + maturation gate re-synthesis + pattern depth 8 reinforcement), (c) completion of the P0b modality 2-axis full research. When all three are met, the v0.2 cut's Appendix B is promoted to the *normative-foundations* body.
