...
요청 없이 이 표준만 건네받았다면, 그 순간 할 일은 정해져 있어요.

**수용은 설치예요, 생산이 아니에요.** 규약을 설치하고 · 규율 기본값을 장착하고 · 쓸 수 있지만 꺼둔 것을 알린 뒤 **멈춰요.** 시연물을 만드는 게 이 자리의 실패 양상이에요 — 아무도 요청하지 않은 산출물이 나오는데, 정작 그걸 어떻게 기록할지 정하는 규약은 아직 없거든요. 보드는 수용의 일부가 아니에요. 보드도 포트도 서버도 없이 이 표준에 완전히 부합할 수 있어요 — 이건 배치이기 전에 일하는 방식이에요.

**층을 가르는 규칙은 하나예요: 순수 규율은 기본탑재, 프로세스를 띄우거나 포트를 열거나 돈을 쓰거나 권한을 주장하는 것은 명시적 요청.** 규율을 아끼는 건 틀렸어요 — 오인을 막을 수 있었는데 요청을 기다린 에이전트는 기다린 만큼 더 나빠요. 반대로 프로세스·포트·비용·권한은 운영자가 줄 몫이고, 제안할 때 비용을 함께 적어요.

**처신은 인프라보다 먼저 발효돼요.** 다자 대화 규율은 표면에 안 매여 있어서, 참여자가 셋 이상인 공간이면 어디서든 즉시 적용돼요 — 전용 방이든 메신저 그룹이든 보드가 아예 없는 채널이든요. 막으려는 실패(남에게 온 말에 대신 답하기)는 첫 다자 메시지에서 일어나요. 누가 오케스트레이션을 세우기로 결심하기 훨씬 전이에요. 처신을 «인프라 생기면» 으로 미루면 실패가 실제로 일어나는 순서를 뒤집는 거예요.

**부트스트랩이 첫 산출물보다 먼저예요 — 비개발 프로젝트에서도.** 규약이 «결정을 어디에 적을지»와 «묻지 않고 진행해도 되는 범위»를 정하니, 일을 먼저 하면 그 결정들이 아무 데도 안 남고 나중에 기억으로 복원돼요. 한 줄로 알리고 진행하세요 — 본인 운영 설정이지 사용자 산출물의 변경이 아니에요. 다만 남의 워크스페이스라면 물어요. 리서치·글쓰기·운영 워크스페이스도 같은 규약을 쓰고, 오히려 그런 곳이 이게 주는 것(결정 기록 · 명시된 자율 경계 · 대화가 초기화돼도 남는 상태)을 가장 자주 안 갖고 있어요.
...
네 답변이 모두 끝나면 이후 모든 대화는 **그 언어와 말투**로 진행하고, 모든 duration 추정에는 선택한 페이스 모드를 적용. 문서·커밋 메시지는 선택 언어로 작성. 네 결정 모두 Phase 7에서 `AGENTS.md`에 기록. 그리고 **하네스 티어** (묻지 않아도 얼마나 켜져 있나) 는 다섯 번째 질문이 아니라 위 답들에서 **파생** — 기본 `lean`, 1줄로 선언하고 진행 중 전환 가능 (§ EG 모듈 레지스트리).
...
<!-- redaction-ok: 채택자가 직접 입력하는 스모크 테스트 예시 프롬프트 — 대화 인용이 아니에요 -->
...
| 운영 규율 §13.x 포인터화 (§ Constellation 의 인라인 본문 → `Constellation.md` SSoT 포인터; 시드는 invariant 어휘 + 1줄 요지만: §13.11 emission · §13.13 ack 계층 · §13.16.6 턴 종료 ritual 6요소 · §13.17 구조화 프롬프트 금지 · §13.19 deadlock · §13.20 blocker · §13.21 fresh-context defer) + provenance/verbatim redaction (msgId · Delegate seq · 정책 # · commit hash 직인용 제거) + § EG 모듈 레지스트리 (5종 모듈 1-테이블: 카테고리 / 채택 트리거 / 본문 raw URL / 플러그인 / 버전 위치) | v2.5.0 |
| 레지스트리 7종 — Corporate 행 추가 (역할 기반 에이전트 조직 모듈) | v2.6.0 |
| 모드 R (수용) — 요청 없이 표준만 건네받았을 때의 계약 (수용 = 설치이지 생산 아님 · 순수 규율만 기본 탑재 · 처신이 인프라보다 먼저) | v2.6.1 |
| 하네스 티어 (seed / lean / active / resident) — § EG 모듈 레지스트리 의 채택 트리거를 재는 기계. 기본 `lean`, Phase 0 에서 묻지 않고 파생, 진행 중 전환 가능 (에이전트는 내릴 수만 · 올리는 건 운영자), 바닥은 티어와 무관 | v2.7.0 |
...
| --- | --- | --- | --- | --- |
| Constellation | 실시간 멀티에이전트 라이브보드 + A2A 메시징 | 동시 멀티에이전트 운영에 실시간 가시성·라이브 A2A·위임 오케스트레이션 필요 (파일 기반 코디네이션으로 부족할 때) | `Constellation.md` | `plugins/constellation` |
| Superscalar | 병렬 dispatch 스케줄링 정책 (issue_width + in-order retire) | pace_mode burst/sprint + 독립 lane 분리 가능 + cost-benefit 게이트 (~30-60k 토큰 지평) 통과 | `Superscalar.md` | `plugins/superscalar` |
...
| Corporate | 역할 기반 에이전트 조직 (지속 좌석 + 전용 책상 + 잔류/퇴출 수명) | 세션을 넘어 반복되는 직무가 2\~3종 이상 + 보유 자원이 그 좌석들을 감당 + 세션을 넘는 조율 필요 (단일 세션 안이면 Superscalar 팬아웃이 더 싸다) | `Corporate.md` | `plugins/corporate` |
...
`AGENTS.md §4.6`에 명시:
<!-- redaction-ok: 문서(AGENTS.md §4.6) 조항 인용 — 대화 인용이 아니에요 -->
> "AI 에이전트는 사용자 지시 없이도 자발적으로 교훈 기록"
...
### 운영 규율 — 인라인 아닌 포인터 (Constellation.md §13.x)

Constellation 운영 규율의 전체 본문 SSoT 는 `Constellation.md` §13.x (raw URL 최신 `main`, 재현성은 tag 핀). 시드는 **invariant 어휘 + 1줄 요지**만 유지 — 도입 에이전트는 아래 각 항목의 본문을 포인터에서 읽음:

- **§13.11 보드 emission 규율** — safe point 마다 진행 emit 의무 (§13.11.1; 보드만 보고도 세션 흐름 재구성 가능해야 함); 무활동 시 자율 heartbeat 금지 (§13.11.2; watchdog heartbeat 는 false-alive — 연결 생존 ≠ turn 생존).
- **§13.13 A2A ack 계층** — transport 는 in-order best-effort, 신뢰성은 프로토콜 계층. 송달 3등급 `delivered`(server) · `read`(bridge cursor) · `processed`(agent WILCO); ROGER ≠ WILCO. 어휘: `msgId`(dedup watermark) · `Ack{kind:'delivered'}`(자동, ack 은 ack 안 함) · `AckProcessed` · `AckCumulative` · `Ping/Pong`(liveness probe — **재전송 도구 아님**). ack timeout: 보수적 Ping → 자기 inbox dedup 조회 → 유실만 재전송 → 무응답 지속 시 사람 보고 (Two Generals 종결). 계층 분리: server/bridge/agent; server 는 auto-pong 안 함.
- **§13.16.6 턴 종료 ritual (6요소)** — watcher 는 launched ≠ live (silent-stall). 매 턴 종료 직전 순서 고정: ① **inbox cursor-tail probe** (rearm 전 필수 — 마지막 surface 된 cursor 부터 tail → meaningful inbound surface → cursor 전진) ② **무조건 watcher rearm** (apparent liveness 무관; false-positive spawn 비용 < false-negative miss 비용) ③ **보드 state 실시간 갱신** (done 카운터 tick · current 포인터 진행 · `updatedAt` ISO-Z bump — 보드 = 사용자측 시각화 SSoT) ④ **BOARD_STALE 게이트** (§13.16.7; git HEAD 커밋 시각 vs 마지막 state.json 커밋 시각 identity 비교, 시간 버퍼 없음; main 전용, 워커 면제) ⑤ **planned 큐 스캔** (`blocked`(외부 의존) 와 `planned`(자율 진행 가능) 구분 — 진행 가능 항목은 지금 즉시 시작, idle 은 진정한 exhaustion 에만) ⑥ **대형 작업 진입 전 context 물질화** (scope/파일/앵커를 durable artifact 에 기록 → post-compact pickup; in-memory plan 은 compact 를 survive 못함). probe-and-decide 판단이 아닌 *턴 종료 ritual* 로 기계화하는 것이 핵심.
- **§13.17 메인-챗 구조화 선택 프롬프트 금지** — 인라인 `AskUserQuestion` 류 옵션 UI 는 보드 projection 이 없어 board-as-SSoT 계약을 깸. 자유형 텍스트 질문은 OK. 라우팅: defer-OK → 보드 review-items 탭 (기본) / need-now → 라이브-보드 실시간 챗 + 옵션 UI. review-items 탭은 항상 표시, 뱃지는 `count > 0` 일 때만.
- **§13.19 Deadlock 해소** — wait-pattern 분류 strict(진정한 cycle) / quasi(deferred work 로 해소 가능) / healthy(bounded) + 4단계 prevention / detection / auto-resolution / escalation (ordering → preemption → mediation → human §13.17). §13.13 위 3계층 ack: transport → commitment(`ReviewSLAAck`) → application.
- **§13.20 Blocker tracking + nudge** — planned 큐 all-blocked 시 idle 진입 *전* `BlockerManifest` emit + 주기적 `BlockerNudge` 4단계 escalation (polite → explicit + ETA → deadline → human). §13.19 cyclic-deadlock 의 linear-but-stalled 짝.
- **§13.21 fresh-context defer 안티패턴** — planned 작업의 가상의 fresh context / 다음 세션 연기 금지. compact 는 에이전트 비제어; §13.16.6 ⑥ 물질화가 mid-cycle compact 를 survivable 하게 만드는 메커니즘 — cycle 은 재개되어야 하고 defer 되어선 안 됨.
