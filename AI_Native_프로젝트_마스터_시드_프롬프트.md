...
<!-- seed-tier: Master; language: Korean; version: v1.4.0; date: 2026-05-03; counterpart: AI_Native_Project_Master_Seed_Prompt.md; changelog: README.md -->
...
사용자 답변을 받으면 이후 모든 대화는 **그 언어와 말투**로 진행. 문서·커밋 메시지는 선택 언어로 작성. 이 두 결정을 Phase 6에서 `AGENTS.md`에 기록.
...
사용자 답변에 따라 Phase 6에서 해당 폴더만 스캐폴딩.
...
선택된 서비스만 Phase 6에서 해당 진입 파일 생성.
...
이 단계에서 **실제 파일들을 생성**합니다. 각 파일 생성 시 한 줄 요약을 사용자에게 보고:

```
생성됨: AGENTS.md — 공용 에이전트 규약 SSoT
생성됨: README.md — 프로젝트 첫인상
생성됨: .agent/rules.md — 작업 규칙 기본
생성됨: .agent/_coordination/STATE.md — 실시간 작업 보드
... (계속)
```
...
합의 후 `.agent/PM/001_Phase1_Plan.md` 생성.

### Step C (Advanced+) — 기술 스택 + 아키텍처 확정

Phase 2에서 논의한 스택을 **확정하고 다이어그램으로 그림**:

- `.agent/architecture.md`에 기술 스택 표 + 데이터 흐름 Mermaid 다이어그램
...
- `.agent/PM/002_MVP_Scope.md`에 기록
- `.agent/PM/003_WBS.md`에 Work Breakdown Structure (태스크 단위까지 분해)
- 각 태스크에 담당 파트(PM/FE/BE)와 예상 시간 태깅

---

## 파일 스캐폴딩 체크리스트

Phase 7 Step A에서 생성할 파일 목록. 각 파일의 템플릿은 **§ 파일 템플릿**에 있음.
...
- [ ] `.agent/PM/README.md`
- [ ] 역할별 폴더 (Phase 2 스택에 따라 `Frontend/`, `Backend/`, `Mobile/`, `Data/` 등)
...
4. `.agent/_coordination/STATE.md` — 실시간 에이전트 활동
...
### 4.1 작업 시작 전 (항상)
1. `.agent/_coordination/STATE.md` 읽기 — 타 에이전트 활동 파악
2. 겹치는 영역이 있으면 `.agent/_questions/open/`에 질문 파일 생성
3. 내 작업을 `STATE.md`에 row 추가

### 4.2 공유 파일 편집 전
- 크로스커팅 파일은 `.agent/_coordination/HANDOFF.md`에 claim
- 기존 claim 확인 → 충돌 시 대기 또는 `_questions/`로 조율

### 4.3 인터페이스 변경 시
- `.agent/_contracts/` 아래 파일 수정, DRAFT → REVIEW → ACTIVE
...
### 4.5 작업 완료 시
- `.agent/_coordination/CHANGELOG.md`에 1줄
- `STATE.md`의 내 row 제거

### 4.6 트러블슈팅 경험 기록 (자율 발전)
- 30분+ 태운 블로커는 `.agent/_lessons/NNN_*.md`에 기록
...
2. **문서화 (3자리 순번 & Index)**: 작업 내역은 `.agent/[파트]/001_Task.md`. README 동시 갱신
3. **Git**: 커밋 규약은 §7, `git commit -a` 금지 (항상 `git add` → `git commit`)
4. **코디네이션 우선**: STATE.md 확인 → 작업 → CHANGELOG.md 기록
5. **트러블슈팅 경험 누적**: 같은 삽질 방지 위해 `.agent/_lessons/`에 기록
6. **3계층 문서 분리 유지**: `.agent/`(에이전트) / `docs/`(인간 개발자) / `executive-docs/`(사업)
7. **인덱스 동기화 (필수)**: `executive-docs/*.md` 등 본문 문서를 추가·재명명·폐기·대규모 재작성할 때 **그 문서를 가리키는 모든 인덱스를 같은 커밋에 갱신**. 일반적인 3-way 세트: (a) 해당 폴더의 `README.md` (카테고리 표); (b) 프로젝트 루트 `README.md` (최상위 진입점); (c) "Living document cycle" 등록부 (있을 경우). 한 곳 누락 시 진입점이 끊기고 다른 에이전트가 옛 목록으로 작업. 상세는 § 인덱스 동기화 정책 참조.
8. **외부 인터페이스 N-way sync (필수)**: 한 기능이 N 개 표면 (AI 스킬 마크다운 · JSON 스펙 endpoint · 개발자 install 가이드 · 사용자 도움말 · 전략 문서 등) 에 묘사될 때, 모든 N 개 표면을 같은 작업 단위에서 갱신. § 외부 인터페이스 N-way sync 의 표를 참조해 무엇이 결합돼 있는지 확인. 실측 사고: 외부 AI 가 일주일 동안 오래된 안내로 작동, 잘못된 필드값을 사용할 뻔.
9. **마크다운 `~` escape (필수)**: GFM 렌더러가 `~text~` / `~~text~~` 를 *취소선* 으로 해석. 본문의 단일 `~` (범위 표기 `2,500\~3,000`, 근사 `\~5분`, Phase 표기 `Phase 4\~5`) 가 한 줄에 두 번 이상 나오면 사이 텍스트가 의도치 않게 취소선 처리됨 → **반드시 `\~` 로 escape**. 외부 HTML/PDF 빌드 직전 escape 스크립트 실행. 상세는 § 마크다운 `~` escape 정책 참조.
10. **RAG 친화 인덱스 키워드 풍부성 (권장)**: 프로젝트가 retrieval-augmented chat 환경 (Claude 프로젝트 파일 공간 100%+ 등) 에 로드될 때, *인덱스 문서* (루트 README · 폴더 README · 본 AGENTS.md · `.agent/rules.md`) 의 키워드 풍부성이 repo 검색 hit-rate 결정. 인덱스 작성·편집 시 5 풍부성 원칙 (본문 명사 ≥3 회 · 약어 + 풀어쓰기 + 모국어 동의어 · 고유명사 · 숫자/날짜 · 프로젝트 커스텀 역할/alias) 적용. 상세는 § RAG 인덱스 최적화 참조.
...
- [.agent/_lessons/README.md] — 트러블슈팅 경험
```

### 브릿지 파일 템플릿

각 AI 서비스의 진입 파일은 아래 형식으로 **동일 패턴**을 따릅니다. 서비스별 고유 기능이 있으면 아래에 추가 섹션.
...
- 워크플로우 명령어: `.agent/workflows/` 참조
- Co-Authored-By: `Claude Opus 4.7 (1M context) <noreply@anthropic.com>` (모델 혼동 주의)

## 공용 규약 요약
작업 시작 전: AGENTS.md → .agent/rules.md → .agent/architecture.md → .agent/_coordination/STATE.md → .agent/_contracts/
```
...
- "5 개 `_lessons/` 파일 요약: 5 병렬 subagent vs 순차 — 병렬 진행 (\~2분 절감). redirect 환영."
