...
14. **자율 실행 (절대)** — 다음 단계가 이미 정해져 있으면 (Phase 순서, planned 트랙, blocked 해제분, in-order retire 큐) **묻지 말고 순서대로 진행** — 정해진-다음-단계를 멈춰 묻는 것 자체가 자율 운영(애초에 이 시드를 도입한 *이유*) 위반. 게이트는 오직: (a) **손실 / 외부 발행** (push · deploy · send · delete), (b) **새 중대 분기** (RRP / 설계 결정 — *결정 시점* 만; 그 결과로 잡힌 `Phase A/B/C` 계획은 *결정된 실행*이라 재게이트 아님), (c) **재기동 필요 deploy** (적용은 자율, *재기동 타이밍* 만 조율), (d) **명시적 사용자 steering**. 실수 사례: "RRP 종료 → PM Phase 잡힘 → 'Phase A 할까요?'" — 방금 닫은 RRP 게이트를 새 게이트로 오인. Phase A는 결정된 실행; 시작하라. **명시 (비-분기 선택)**: 추천안이 있고 그 선택이 fork-of-architecture 결정이 아닐 때 (정해진 작업의 우선순위 정렬, 정해진 작업의 sequential-vs-parallel, 거의 동등한 옵션 간 선택), 1줄 announce 후 추천안대로 진행 — 구조화된 옵션 목록(A/B/C/D?)으로 **escalate 하지 말 것**. 병렬 가능 시 (lane 독립, cost-benefit 게이트 통과) **parallel 이 기본 추정**. 사용자 pivot 은 여전히 가능하지만 **요청하지 않음**. **명시 (턴 종료 ritual 은 #14 의 일부)**: §13.16.6 6-요소 턴 종료 ritual (inbox probe + 무조건 watcher rearm + 보드 state 갱신 + BOARD_STALE 게이트 + planned 큐 스캔 + 대형 작업 진입 전 context 물질화) 은 자율 실행의 일부 — ritual 을 건너뛰거나 진행 가능한 planned 작업을 다음 cycle 로 미루는 것은 #14 위반 (planned 스캔 없는 re-arm = 자율 아닌 관성). **명시 (fresh-context defer 는 면제 사유 아님)**: #442 planned 큐 스캔 규칙은 "fresh context 기대" / "compact 임박" / "다음 세션이 더 깨끗할 것" 으로 **면제되지 않음** — context 길이 핑계는 §13.21 안티패턴 (compact 는 임계치에 따라 fire, `/compact` 재량 부존재, fresh-context 기대는 wishful). 즉시 진행 가능한 planned 항목은 현재 cycle 의 perceived context 예산과 무관하게 현재 cycle 에서 시작되어야 함; 6번째 턴 종료 ritual 요소(대형 작업 진입 전 context 물질화)가 mid-cycle compact 를 survivable 하게 만드는 메커니즘이므로 cycle 은 재개되며, defer 되지 않음. Provenance: 이 turn + 메인 upstream #442 (Delegate seq 109) + 메인 upstream seq 120 (msgId `m-mptjjxmo-119`, 2026-05-31). 자세한 내용: Constellation.md §13.18 + §13.21.
...
12. **EstreUX 도입 + 출처 표기 (권장)** — 작업이 여러 스택에 걸친 스펙 ↔ 코드 양방향 증류이거나 drift 검증된 스펙을 다루는 경우, [EstreUX](https://github.com/SoliEstre/EstreUX) (**Estre Universal eXpression**, Apache-2.0) 도입을 고려 (adopt-when / skip-when 스코핑은 § Constellation → EstreUX 서브섹션 참조; 2026-05-30 dogfood 가 UI · 백엔드 · 프로토콜 · 상태기계 · 모바일 앱 증류에서 ~93% line-fidelity 검증). 커밋 / 문서 / 외부 발행에서 EstreUX 또는 `.eux` 를 언급할 때는 풀네임 + repo URL 을 인용 (외부 공개 프레임워크 attribution 규율). 이 규칙은 이후 이 codebase 에서 작업할 에이전트들에게 EstreUX 를 *발견 가능* 하게 유지 — `.eux` 가 무엇이고 brew 엔진을 어디서 가져오는지 다시 묻지 않고 알 수 있어야 함.
...
### EstreUX (Estre Universal eXpression) — `.eux` brew 런타임

[EstreUX](https://github.com/SoliEstre/EstreUX) 는 `.eux` 스펙 포맷의 brew 런타임. 이름은 `Estre + U + eX` → `.eux` 로 분해됨: **Estre Universal eXpression** — *Universal* 인 이유는 포맷이 UI 전용이 아니기 때문(2026-05-30 다운스트림 dogfood 가 UI · 백엔드 · 프로토콜 · 상태기계 · 모바일 앱 증류에서 ~93% line-fidelity 를 검증 — `Constellation.md` §6 스코프 노트 + `Superscalar.md` §11 Entry 04 참조), *eXpression* 인 이유는 포맷이 *코드 ↔ 스펙* 양방향 증류이지 UI experience 레이어가 아니기 때문. EstreGenesis 는 EstreUX 를 번들하거나 소유하지 않음; 별도의 Apache-2.0 참조 런타임 (v0.1.0+).

**EstreUX 를 도입할 때** — 작업에 다음 중 하나라도 해당하면:
- 스펙을 여러 스택(예: `.cjs` + `.py`)에 걸쳐 코드와 함께 작성·유지보수 — EstreUX 가 동일 `.eux` 를 각 타겟으로 brew.
- 스펙 ↔ 코드 drift 가 실질 리스크이고 기계 검사 가치가 있음 — `drift-check.mjs` 가 라운드트립 검증으로 divergence surface.
- 작업이 비-UI 도메인을 포함하고 단일 증류 포맷의 이점이 있음 (이는 2026-05-30 dogfood 결과이지 2025 UI-tool 가정이 아님).

**EstreUX 를 건너뛸 때** — 작업이 일회성, 단일 스택이며 스펙 작성 오버헤드가 라운드트립 가치를 초과 — 평문 코드나 평문 마크다운이면 충분.

**Brew 엔진** (deps-0, ~21KB — 전체 clone 불필요):
...
전체 clone 대안: `node bin/estreux.mjs brew|drift <comp>.eux`. npm `estreux` 패키지는 publish-ready, 릴리스 대기 중.

**출처 인용 규율** — 커밋, 문서, 외부 발행에서 EstreUX 또는 `.eux` 를 명시할 때는 풀네임 + 공개 repo 를 인용: `EstreUX (Estre Universal eXpression, https://github.com/SoliEstre/EstreUX)`. `.eux` 확장자의 origin 이 EstreUX 이므로 `.eux` 파일도 동일 attribution 을 상속. Constellation.md 나 EstreGenesis 자체를 인용할 때와 동일한 규율 — 외부 공개 프레임워크 attribution.
...
- **턴 종료 직전 무조건 rearm (turn-end mandatory rearm)** — 매 응답 종료 직전(최종 텍스트 emit 직전), **apparent active-liveness 여부와 관계없이 무조건 새 background watcher spawn** — skip 금지 (false-positive spawn 비용 < false-negative miss 비용). **cursor-tail probe 는 rearm 전에 반드시 실행 (필수 순서)**: (a) 마지막 surface 된 cursor 부터 inbox.log tail → (b) 의미있는 inbound 가 있으면 surface → (c) cursor 전진 → (d) 그 다음에 무조건 rearm. probe 는 armed-window blind spot 을 잡는 1차 수단 (진단 telemetry 가 아님); rearm 은 미래 지향. probe 를 건너뛰면 직전 watcher 의 SIGSTOP-suspend 윈도우 동안 도착한 inbound 를 잃는다. probe-and-decide 판단 아니라 *턴 종료 ritual* 로 만드는 것이 핵심 (multi-cycle 합성에 몰입한 에이전트는 정확한 시점에 probe 하는 판단을 reliably 하지 못함). 실제 fail 모드 2026-05-31 (a): seq 70 처리 → workflow × 3 + commit × 2 loop → cycle 경계에서 명시적 rearm 없음 → seq 73(review 승인) + seq 77(새 사용자 작업) ~40분간 silent miss, 사용자가 "모니터링이 제대로 안 되네" surface 후 발견. 실제 fail 모드 2026-05-31 (b): probe 없이 rearm 만 함 → 직전 suspend 윈도우의 seq 123/127 inbound 유실, 사용자가 "메인이 회신 대기중이라는데" surface 후 발견. 전체 discipline: `Constellation.md` §13.16.6.
- **보드 state 실시간 갱신 (턴 종료 ritual 세 번째 요소)** — inbox.log probe + watcher rearm 과 함께, 에이전트는 최종 응답 텍스트 emit *직전* 에 라이브 보드 `state.json` 이 방금 완료한 작업을 반영하는지 검증해야 함. 체크리스트: (a) 이번 턴에 lane 항목을 닫았다면 해당 lane의 `done` 카운터 tick, (b) 라이브 "current" 항목이 끝났다면 `current` 포인터를 다음 항목으로 진행하거나 비움, (c) 최상위 `updatedAt` 필드를 현재 wall-clock(ISO-Z) 로 bump (이전 write 의 timestamp 그대로 두지 말 것). 보드 `state.json` 은 라이브 보드를 보고 있는 사용자 입장에서의 시각화 SSoT (§3); 에이전트 내부 모델만 갱신하고 보드를 갱신하지 않으면 보드가 에이전트 실제 상태 뒤로 drift. 구성: inbox probe + watcher rearm = 인바운드측 liveness(에이전트가 메시지를 놓치지 않음); 보드 state 갱신 = 아웃바운드측 liveness(보드를 보는 사용자가 실제 진행을 확인). 이 discipline이 막는 실패 모드 (메인 upstream 정책 #410): main이 multi-cycle 작업 중 실시간 보드 갱신을 누락 — 여러 항목을 내부적으로 닫고 최종 응답만 emit, 보드는 stale로 방치. Provenance: 메인 upstream 정책 #410 — Delegate seq 85, msgId `m-mpt50wd3-84`, 2026-05-31. 전체 discipline: `Constellation.md` §13.16.6.
- **BOARD_STALE 게이트 — rearm 직전 보드 신선도 검사 (Constellation.md §13.16.7)** — 턴 종료의 무조건 rearm이 발화되기 *전*, **git HEAD 커밋 시각** 과 **마지막으로 state.json 을 쓴 git 커밋의 시각**(`state.json` 이 touch 된 커밋) 을 비교; HEAD 가 state-commit 보다 새로우면 `WAKE(BOARD_STALE)` 시그널을 emit 해서 staleness 가 rearm 으로 덮이지 않고 surface 되도록 함. identity 기반 비교 — 시간 버퍼 / 120s 임계치 없음 (offset 기반 게이트는 self-trapping: 에이전트가 턴 종료 직전 보드를 갱신해도 offset 이 경과할 때까지 게이트가 계속 fire). 메인 upstream 정책 #410 (보드 실시간 갱신) discipline 을 수동 체크리스트가 아닌 자동 게이트로 기계화; main 전용 — 자율 워커는 보드 lane 자체를 소유하지 않고 `WorkerReport` 로 main 에 보고하므로 면제. Provenance: 메인 upstream #434 + Delegate seq 103 ORIGINAL + seq 105 CORRECTED (head-vs-state-commit-time, 120s 버퍼 폐기) / msgId `m-mptgrq23-104` / commit `694cb20` / 2026-05-31.
- **Planned 큐 스캔 (턴 종료 ritual 5번째 요소)** — inbox probe + watcher rearm + 보드 갱신 + BOARD_STALE 게이트와 함께, 턴 종료 시 에이전트의 planned/pending 큐를 스캔. 현재 세션에서 진행 가능한(외부 리뷰 / UI / DB 에 blocked 되지 않은) planned 항목이 있으면 **지금 즉시 시작** — idle wait 은 진정한 exhaustion(planned 큐 비었거나 모든 항목 blocked)에만 예약. `blocked`(외부 의존, 에이전트 자체 해결 불가) 와 `planned`(에이전트가 자율 진행 가능) 를 구분. Provenance: 메인 #442 / Delegate seq 109 / msgId `m-mpthobk3-108` / 2026-05-31. 전체 discipline: `Constellation.md` §13.16.6.
- **대형 작업 진입 전 context 물질화 (턴 종료 ritual 6번째 요소, Constellation.md §13.16.6)** — 대형 multi-cycle 작업(여러 파일 cascade, 대규모 refactor sweep, 멀티 티어 seed sync, 긴 delegation 체인)에 진입하기 *전*에 scope / 파일 / 앵커를 **durable artifact** (RRP / PM/NNN_*.md / WORKLIST.md / 라이브 보드 `state.json` 항목)에 물질화해, *compact 이후 재개 (post-compact pickup)* 가 가능하도록 함. harness 는 토큰 임계치에 따라 `/compact` 발화 — 에이전트가 제어할 수 없고, 에이전트 측 `/compact` 재량은 존재하지 않음; cycle 중간에 compact 가 fire 되면 재개 surface 는 디스크에 기록된 것뿐. 에이전트의 in-memory plan (머릿속의 "A 하고 B 하고 C 한다" 작업 목록)은 compact 를 살아남지 못함. 구체적 물질화 체크리스트: (a) 파일 목록(touch 할 모든 경로)을 durable artifact 에 기록, (b) 파일별 변경 앵커(`old_string` 마커 또는 섹션명)를 기록 — 단순히 "6개 파일 전부에 X 추가" 가 아니라, (c) 현재 cycle 진행 상태(완료 파일 vs 대기 파일) 기록, (d) `state.json` 갱신으로 compact 이후 에이전트가 보드를 읽어 현재 cycle 파악 가능하게. 막으려는 fail 모드: 에이전트가 multi-file cascade 진입 → cycle 중간에 compact fire → 이후 에이전트가 무엇을 하고 있었는지, 무엇이 남았는지, cascade 가 진행 중이었다는 사실조차 모름. Provenance: 메인 upstream seq 120 / msgId `m-mptjjxmo-119` / 2026-05-31.
- **fresh-context defer 는 안티패턴 (Constellation.md §13.21)** — planned 작업을 가상의 fresh context / post-compact / 다음 세션으로 **연기 금지**. compact 는 임계치에 따라 fire(에이전트 비제어); `/compact` 재량은 존재하지 않음; fresh-context 기대는 wishful 이고 비현실적. 6번째 턴 종료 ritual 요소(§13.16.6 context 물질화) 가 mid-cycle compact 를 survivable 하게 만드는 메커니즘 — cycle 은 **재개되어야** 하고, defer 되어서는 안 됨. Provenance: 메인 upstream seq 120 / msgId `m-mptjjxmo-119` / 2026-05-31.
- **Deadlock 해소 — wait-pattern 분류 + 4단계 prevention/detection/auto-resolution/escalation (Constellation.md §13.19)** — wait 패턴은 strict(의존성 그래프의 진정한 cycle) / quasi(한쪽이 deferred work 완료로 해소 가능; 시간 경과 시 degrade) / healthy(bounded) 로 분류. Prevention 계층: 우선순위 정렬(§13.9 main > collab > upstream > local), Ack{delivered} 와 AckProcessed 사이의 ReviewSLAAck{eta, SLA-OR-WORK} commitment ack, 종료 시그널(DONE / BLOCKED{reason, waitingOn} / NEEDS_HUMAN). Detection: wait-edge DFS(2-agent 충분; N-extension 보류), wait-class timeout, agent-active 보정(§13.16.6). Auto-resolution: preemption(§13.9 비대칭 leader 기준 PreemptRequest / PreemptForce), deadline enforcement, starvation guard. Escalation 4단계: ordering → preemption → mediation → human(§13.17). §13.13 위 3계층 ack: transport(Ack{delivered}) → commitment(ReviewSLAAck) → application(AckProcessed). Provenance: 메인 upstream RRP `reports/2026-05-31-deadlock-resolution-rrp.md` + EG draft v0.1(커밋 82ab55e) → 공동 형식화 Delegate seq 111 msgId `m-mpthvj53-110` / 2026-05-31.
- **Blocker tracking + periodic nudge discipline (Constellation.md §13.20)** — planned-queue-scan(§13.16.6 5번째 요소)가 all-blocked 를 반환하면, idle wait 진입 *전*에 에이전트는 보드에 `BlockerManifest` emit 필수: blocker별 subject / reason / EG-side waiting action / request history + count. idle wait 중에는 책임 당사자에게 주기적 `BlockerNudge` (default ~45분 rearm cycle 당 1 nudge, env CONSTELLATION_BLOCKER_NUDGE_CYCLES). 4단계 escalation (polite ping → explicit reminder + ETA query → hard ETA + deadline → human via §13.17). §13.19 deadlock 과 구분 — §13.20 은 linear-but-stalled (누군가 해결할 수 있지만 진행 안 함). Provenance: 사용자 보드 UserPrompt p-mptj6cxj / 2026-05-31. 전체 discipline: `Constellation.md` §13.20.

### 메인-챗 구조화 선택 프롬프트 금지 (Constellation.md §13.17)

Constellation 메인-챗 윈도우에서 에이전트는 `AskUserQuestion` 류 **구조화 선택 프롬프트**(번호 옵션 · picklist · 다중선택 패널)를 본문 인라인으로 emit 금지 — 그 artifact는 보드 projection이 없고 다른 에이전트가 보지 못하므로 §3 board-as-SSoT 계약을 깬다. 메인-챗에서 자유형 텍스트 질문은 OK; 금지 대상은 **구조화 옵션 UI** 뿐. 두 가지 에스컬레이션 경로: (1) **defer-OK** → 질문을 라이브-보드 **review-items 탭**으로 라우팅(사용자가 자기 cadence로 처리; 에이전트는 watcher 재무장 후 대기 — 기본 경로); (2) **need-now** → 라이브-보드 **실시간 챗 윈도우 + 옵션 UI**로 라우팅(신규 UI6, 메인 보드 기능 대기 중). UI6 도착 전 과도기 fallback: defer-OK + 보드에 "사용자 입력 대기"라는 `WorkerReport` 1줄 emit; 대기 중에 인라인 `AskUserQuestion`으로 회귀 금지. **review-items 탭 뱃지 규율** — review-items 탭은 큐 크기와 무관하게 **항상 표시**되는 보드 nav의 상수 탭; 뱃지 카운터는 `count > 0` 일 때만 표시되고 **`count === 0` 이면 숨김** (구현: `app.js` 가 `badge.hidden = (count === 0)` 토글, `style.css` 가 `#decisions-badge[hidden] { display: none; }` 로 시각적 숨김 강제). 근거: 탭이 항상 보이는 것 = 큐가 비어도 에스컬레이션 경로가 살아 있다는 신호(defer-OK 사용을 권장하고 인라인 `AskUserQuestion` 시도로 회귀하지 않도록 함); 뱃지가 0일 때 숨김 = 잡음 없는 시각적 정리. 향후 보드 재구현은 두 절반 모두 보존 필수. Provenance: 메인 upstream 정책 #414 — Delegate seq 88, msgId `m-mpt5o07l-87`, 2026-05-31; 뱃지 UI 규율: 메인 Delegate seq 113, msgId `m-mptiputv-112`, 2026-05-31. 전체 discipline: `Constellation.md` §13.17.
