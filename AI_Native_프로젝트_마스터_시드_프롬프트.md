...
- **BOARD_STALE 게이트 — rearm 직전 보드 신선도 검사 (Constellation.md §13.16.7)** — 턴 종료의 무조건 rearm이 발화되기 *전*, git HEAD 커밋 epoch 과 라이브 보드 `state.json` 의 `updatedAt` 을 비교; 차이가 120s 신선도 임계치를 초과하면 `WAKE(BOARD_STALE)` 시그널을 emit 해서 staleness 가 rearm 으로 덮이지 않고 surface 되도록 함. 메인 upstream 정책 #410 (보드 실시간 갱신) discipline 을 수동 체크리스트가 아닌 자동 게이트로 기계화; main 전용 — 자율 워커는 보드 lane 자체를 소유하지 않고 `WorkerReport` 로 main 에 보고하므로 면제. Provenance: 메인 upstream #434 — Delegate seq 103, msgId `m-mptgihb6-102`, 2026-05-31.
