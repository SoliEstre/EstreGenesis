...
**Current**: v2.5.193 (2026-07-20) — **Composer left action group (Constellation v2.4.80)** — the ops status strip and the agent-activity monitor toggle move into a new left-anchored group on the composer action row ([⚙ 🧵] left · send/attach/pause/stop right), Playwright-measured on the live board. verify-nway 20-axis PASS.
...
**현재**: v2.5.193 (2026-07-20) — **입력줄 좌측 액션 그룹 (Constellation v2.4.80)** — 운용 상태 스트립과 에이전트 활동 모니터 토글이 입력줄 액션 행의 신설 좌측 그룹으로 이동해요([⚙ 🧵] 좌측 · 전송/첨부/일시정지/중지 우측). 라이브 보드 Playwright 실측. verify-nway 20축 PASS.
