...
**Current**: v2.5.198 (2026-07-20) — **Adapter contract §5.5 — pristine-copy adopters as a reference↔live drift sensor (Harness Adapter Contract v0.3.1, doc-only)** — the day's three-round bridge thread closed with a durable decomposition worth keeping: a reference impl can silently lag its own live deployment (operational pressure hardens the live copy first; "canonical" attracts less verification pressure), and an adopter running the reference byte-identical is the sensor that reveals it — evidenced by a series where two of three defects were already guarded in the EG live bridge but unfixed in the reference. Captured as a verification principle + a maintainer corollary (check the live copy for the same class AND the reverse). verify-nway 20-axis PASS.
