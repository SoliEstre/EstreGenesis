...
<p align="center">
  <img alt="Version: v2.5.158" src="https://img.shields.io/badge/version-v2.5.158-2ea44f?style=for-the-badge" />
...
**Current**: v2.5.158 (2026-07-18) — **boardsweep learns the consumer axis (constellation plugin v0.3.28)** — an adopter-measured defect class: a stage list written as `{name,state}` against a `{label,status}` renderer survived two sweeps rendering all-PENDING, because the sweep checked repo facts but not the contract the consumer reads. Schema normalization now checks field keys/value domains against the consumer contract — render fallbacks silently swallow exactly this class. verify-nway 19-axis PASS.
...
**현재**: v2.5.158 (2026-07-18) — **boardsweep 이 소비자 축을 배움 (constellation 플러그인 v0.3.28)** — 어댑터 실측 결함 클래스: `{name,state}` 로 쓰인 stage 목록이 `{label,status}` 렌더러 아래 전부 PENDING 으로 스윕 2회를 살아남았어요 — 스윕이 저장소 사실만 대조하고 소비자가 읽는 계약은 대조하지 않았기 때문. schema normalization 이 이제 필드 키·값 도메인을 소비자 계약과 대조해요 — 렌더 폴백이 정확히 이 클래스를 조용히 삼켜요. verify-nway 19축 PASS.
