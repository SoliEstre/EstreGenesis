...
<p align="center">
  <img alt="Version: v2.5.47" src="https://img.shields.io/badge/version-v2.5.47-2ea44f?style=for-the-badge" />
...
**Current**: v2.5.47 (2026-06-04) — phase_3 P3 1st cut: `@invariants` + `@metamorphic` 절 격상 to `history-store.eux` + `server-relay.eux` per eux-format v1.2 §2.6/§2.7 (EstreUX `884552b`). prose+vocabulary form (Q1=c) with 4 sub-sections (`state`/`temporal`/`transaction`/`causality`) using the 17-keyword vocabulary (bounded/ordered/monotonic/member-of · within/after/at-most-once/idempotent/expires-after · atomic/rollback-safe/dual-write/commit-then/persist-before · precedes/iff/implies) + 3 metamorphic sub-sections (round-trip/idempotency/determinism, P4 짝). `history-store.eux` carries DB integrity invariants (mode-B dual-write atomic + rollback-safe · backfillFromJsonl idempotent · exportJsonl byte-identical commit-then · autoRevert persist-before state.json mutation 금지) + 3 metamorphic properties; `server-relay.eux` carries §13.8 reply-window + §13.19 deadlock + sourceStampFallback + main_handoff axis 2 invariants + 3 metamorphic properties. Body prose unchanged; +90 lines added across 2 files. `@machine wsWaitState` (§13.19) formal lift deferred to a separate cut (P3 2nd or v2.5.48+). Seed-tier (Master + Lite + Compact × EN/KO) is at **v2.4.3** (2026-06-03). Most recent module cuts: Constellation v2.3.22, Superscalar v0.4.2, Hyperbrief v0.5.6.
...
**현재**: v2.5.47 (2026-06-04) — phase_3 P3 1st cut: `history-store.eux` + `server-relay.eux` 에 `@invariants` + `@metamorphic` 절 격상 per eux-format v1.2 §2.6/§2.7 (EstreUX `884552b`). prose+vocabulary 형식 (Q1=c) + 4 sub-section (`state`/`temporal`/`transaction`/`causality`) + 17 키워드 vocabulary (bounded/ordered/monotonic/member-of · within/after/at-most-once/idempotent/expires-after · atomic/rollback-safe/dual-write/commit-then/persist-before · precedes/iff/implies) + 3 metamorphic sub-section (round-trip/idempotency/determinism, P4 짝). `history-store.eux` 가 DB 무결성 invariants 운반 (mode-B dual-write atomic + rollback-safe · backfillFromJsonl idempotent · exportJsonl byte-identical commit-then · autoRevert persist-before state.json mutation 금지) + 3 metamorphic property; `server-relay.eux` 가 §13.8 reply-window + §13.19 deadlock + sourceStampFallback + main_handoff axis 2 invariants 운반 + 3 metamorphic property. 본체 prose 무변경; 2 파일 +90줄 추가. `@machine wsWaitState` (§13.19) 정식 격상은 별도 cut 보류 (P3 2nd 또는 v2.5.48+). 시드 tier (Master + Lite + Compact × EN/KO) 는 **v2.4.3** (2026-06-03). 최근 모듈 cut: Constellation v2.3.22, Superscalar v0.4.2, Hyperbrief v0.5.6.
