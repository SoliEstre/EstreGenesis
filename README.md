...
**현재**: v2.5.164 (2026-07-19) — **실시간 기록 발송시각이 더는 "현재 시간"으로 안 튐 (Constellation v2.4.60)** — 68행이 실제 발송시각을 ISO 문자열로 갖고 있었는데 숫자-전제 소비자(대시보드 epoch 파서·부팅 정렬 `timestamp||0`)가 새로고침마다 현재시간으로 격하시키던 버그 — `wsNormTs` 가 저장·적재 경계에서 강제(문자열→epoch·누락→수신시각)하고 대시보드도 문자열을 수용해요. 유추 아닌 실시각 복원, 재시작 1회로 68→0 실측. verify-nway 20축 PASS.
