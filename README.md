...
<p align="center">
  <img alt="Version: v2.5.45" src="https://img.shields.io/badge/version-v2.5.45-2ea44f?style=for-the-badge" />
...
**Current**: v2.5.45 (2026-06-04) — phase_2 modular `.eux` split per eux-format v1.1 (EstreUX commit 77e12f9): `gateway-client.eux` (264L, single-file `metadata:` 12-dim form) split into parent manifest + 4 `@deps` sub-modules (`-handshake` connection lifecycle, `-a2a` envelope/routing/delivery/ack-tier with Hyperbrief `decided` extension, `-attachments` §13.11 rule 5 formalized from prose, `-key-mgmt` §13.16.11 client-side formalized from prose); `server.eux` (97L) split similarly into parent + 4 (`-core` HTTP/WS/envelope_convention, `-relay` A2A/sourceStamp/§13.19 deadlock, `-keys` KEY-MGMT + C7 `@hazards` credential, `-history` per-channel ring + DB P2 thin shim); `history-store.eux` gains C7 `@hazards` DB integrity surfaces. Constellation.md §6 cross-link updated. Closes phase_1 G3/G6 evaluation outcomes per hub gap catalog 2026-06-03. Seed-tier (Master + Lite + Compact × EN/KO) is at **v2.4.3** (2026-06-03). Most recent module cuts: Constellation v2.3.22, Superscalar v0.4.2, Hyperbrief v0.5.6.
...
**현재**: v2.5.45 (2026-06-04) — eux-format v1.1 (EstreUX commit 77e12f9) 기반 phase_2 모듈 분리 ship: `gateway-client.eux` (264줄 single-file `metadata:` 12-dim 형식) 를 부모 manifest + 4 `@deps` sub-module 로 분리 (`-handshake` 연결 lifecycle, `-a2a` envelope/routing/delivery/ack-tier with Hyperbrief `decided` 확장, `-attachments` §13.11 rule 5 prose → 정식화, `-key-mgmt` §13.16.11 client-side prose → 정식화); `server.eux` (97줄) 동일 분리 (`-core` HTTP/WS/envelope_convention, `-relay` A2A/sourceStamp/§13.19 deadlock, `-keys` KEY-MGMT + C7 `@hazards` credential, `-history` 채널별 ring + DB P2 thin shim); `history-store.eux` 에 C7 `@hazards` DB 무결성 표면 추가. Constellation.md §6 cross-link 갱신. phase_1 G3/G6 평가 결과 close (허브 gap catalog 2026-06-03). 시드 tier (Master + Lite + Compact × EN/KO) 는 **v2.4.3** (2026-06-03). 최근 모듈 cut: Constellation v2.3.22, Superscalar v0.4.2, Hyperbrief v0.5.6.
