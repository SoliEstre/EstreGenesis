...
**Current**: v2.5.182 (2026-07-19) — **turn-spawned local worker profile (Constellation v2.4.72 §13.27.5)** — the loop contract's worker profile: supervisor/turn split, charter additions, id-idempotency, session reuse with rotation, silence as output, escalation, state-measured completion. verify-nway 20-axis PASS.
...
**현재**: v2.5.182 (2026-07-19) — **턴-스폰 로컬 워커 프로필 (Constellation v2.4.72 §13.27.5)** — 루프 계약의 워커 프로필: 수퍼바이저/턴 분리·차터 추가항·id-멱등·세션 재사용+로테이션·침묵=유효 출력·에스컬레이션·완료=state 실측이에요. verify-nway 20축 PASS.
