...
**Current**: v2.5.168 (2026-07-19) — **activity-monitor UX + noise refinement (Constellation v2.4.64)** — the toggle fab drops to the realtime-fab layer (no longer floats over the pane), a fixed-width `Wn/n Sn` indicator toggle joins the prompt-row's left edge (live-verified `W2/2 S1`), and the 15-second snapshot events are excluded from Web Push and the worker's pending classifier — the measured worker-turn cascade is stopped. verify-nway 20-axis PASS.
...
**현재**: v2.5.168 (2026-07-19) — **활동 모니터 UX + 소음 정련 (Constellation v2.4.64)** — 토글 fab 을 실시간 fab 레이어로(창 위로 안 뜸), 입력줄 왼쪽에 고정 폭 `Wn/n Sn` 인디케이터 토글(실측 `W2/2 S1`), 15초급 스냅샷 이벤트를 Web Push·워커 pending 분류기에서 제외 — 실측된 워커 턴 캐스케이드 종식. verify-nway 20축 PASS.
