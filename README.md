...
<p align="center">
  <img alt="Version: v2.5.159" src="https://img.shields.io/badge/version-v2.5.159-2ea44f?style=for-the-badge" />
...
**Current**: v2.5.159 (2026-07-18) — **room artifacts + the board state schema becomes canon (Constellation v2.4.57, plugin v0.3.29)** — rooms get their three pinned artifacts server-side (header · append-only decision ledger · running summary with `covers_until`) with fetch/write contracts (`RequestRoomArtifacts` / `RoomArtifactsUpdate`, delta broadcast under an ack-by-reference version), a collapsible 📌 dashboard block, and a scribe digest duty; §13.31 (new) makes the board data contract canonical — `current[].stages {label, status: pending|active|done}` plus a usage discipline and charter/sweep/skill hooks — after a renderer-code-only contract proved silently fragile. Smoke 22/22, verify-nway 19-axis PASS.
