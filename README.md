...
**Language**: **English** (below) · [한국어](#estregenesis--ai-native-프로젝트-시드-프롬프트-라이브러리-한국어)

> **A tiered prompt system for bootstrapping and migrating AI-native software projects** — collaboration patterns designed for the era when Claude, Gemini, Cursor, Copilot, Windsurf, and other AI coding agents all work on the same codebase, sometimes concurrently.
...
Each language pair (English + Korean) of a given tier is fully aligned — same phases, same migration logic, same operational guidance. Pair them if your team is bilingual.
...
5. Review the extraction, approve, let the agent rewrite bridge files to import `@AGENTS.md`, reorganize `.agent/`, and record the migration in `.agent/_lessons/001_AI_Native_Migration.md`.

### Scenario 3 — existing project, already seed-bootstrapped with an older version
...
Every bridge is a one-line `@AGENTS.md` import plus whatever service-specific knobs that tool requires. Switching services is a no-op — the SSoT stays.
...
# EstreGenesis — AI Native 프로젝트 시드 프롬프트 라이브러리 (한국어)

**언어**: [English](#estregenesis--ai-native-project-seed-prompt-library) · **한국어** (아래)

> **Claude, Gemini, Cursor, Copilot, Windsurf 등 여러 AI 코딩 에이전트가 같은 코드베이스에서 — 때로는 동시에 — 작업하는 시대를 위한 AI-Native 소프트웨어 프로젝트의 부트스트랩·마이그레이션 티어 프롬프트 시스템.**
...
각 tier 의 언어 페어(영문+한글)는 완전 정렬 — 동일한 phase, 동일한 마이그레이션 로직, 동일한 운영 가이드. 이중언어 팀이면 페어로 배치.
...
5. 추출 내용 검토·승인, 에이전트가 브릿지 파일들을 `@AGENTS.md` import 로 재작성, `.agent/` 재조직, `.agent/_lessons/001_AI_Native_Migration.md` 에 마이그레이션 기록.

### 시나리오 3 — 기존 프로젝트, 구 버전 시드로 부트스트랩됨
...
│   └── PM/                            ← 계획, 3자리 접두어
│       └── 001_Phase1_Plan.md
├── CLAUDE.md                          ← Claude Code 브릿지 (@AGENTS.md import)
...
모든 브릿지는 `@AGENTS.md` import 한 줄 + 해당 도구 특화 설정. 서비스 전환은 no-op — SSoT 유지.
