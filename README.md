...
<p align="center">
  <img alt="Version: v2.5.102" src="https://img.shields.io/badge/version-v2.5.102-2ea44f?style=for-the-badge" />
...
**현재**: v2.5.102 (2026-06-13) — **Constellation v2.4.31 — 모바일 하단 탭바 (실시간이 탭으로 합류)** — 확정한 모바일 네비 재구성의 구조 절반(A). ≤560px 에서 하단 고정 탭바 `[대시보드][검토사안][위키][실시간]` 가 떠서 최상위 네비가 됨: 상단 `.topbar` 탭과 플로팅 💬 FAB 은 숨기고, **실시간 뷰(기존 플로팅 팝업)가 하단 탭으로 합류** — 실시간 선택 시 풀스크린 실시간 pane 열기, 대시보드/검토사안 선택 시 닫고 해당 pane 표시. 탭바 active 는 `ui.panes`+`popOpen` 에서 동기(`applyPanes`+`toggleWsPop` 훅). 위키는 #4 Compendium 자리(disabled). 실시간 pane 은 탭바 공간 남김(하단 inset), `main` 하단 패딩, home 버튼 탭바 위로. **데스크탑(>560px) 무영향** — 상단 탭 + 플로팅 팝업 그대로. 모바일 한정, 실기기 테스트 필요. 남은 #3a: (B) 실시간 탭 안 그룹단위 스와이프. 레퍼런스 대시보드(index.html/app.js/style.css), 프로토콜 표면 0. verify-nway 11축 PASS.
