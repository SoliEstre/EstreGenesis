...
**Current**: v2.5.187 (2026-07-20) — **selection timeout (Constellation v2.4.74)** — SelectionPrompt gains the clarify/approval gate polarity with server-owned expiry (SelectionExpired wakes turn-based issuers via at-least-once); late clarify answers are steering. verify-nway 20-axis PASS.
...
**현재**: v2.5.187 (2026-07-20) — **선택지 타임아웃 (Constellation v2.4.74)** — SelectionPrompt 에 분기/승인 극성 + 서버-소유 만료(SelectionExpired 가 at-least-once 로 턴-기반 발신자를 깨워요); 분기의 늦은 답은 스티어링이에요. verify-nway 20축 PASS.
