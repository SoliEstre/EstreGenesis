...
**현재**: v2.5.189 (2026-07-20) — **CapabilityManifest — 계약-표면 선언 (Constellation v2.4.76)** — 선언-이벤트 클래스의 세 번째 멤버: 에이전트가 자신이 구현한 와이어 계약 표면(`selection`·`roundtable` 등)을 latest-wins 서버 영속 + History 동봉으로 광고해요 — 표면을 어둡게(dark) 실은 이종 하네스 어댑터의 opt-in 이 탐침 대신 선언된 상태로 협상돼요. verify-nway 20축 PASS.
