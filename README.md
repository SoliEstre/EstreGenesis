...
**Current**: v2.5.167 (2026-07-19) — **the inspector becomes an agent-activity monitor (Constellation v2.4.63)** — a SubagentStatus section (standalone subagents, 10-minute activity window measured off a 6-minute tool-call flush gap), a fixed 🧵 fab toggling the layer with an overview mode, and real theme-token styling so the window follows light/dark (the old styling was pinned to dark fallbacks of nonexistent variables). E2E incl. live data from a real running delegation + theme flip measurement. verify-nway 20-axis PASS.
...
**현재**: v2.5.167 (2026-07-19) — **인스펙터가 에이전트 활동 모니터로 (Constellation v2.4.63)** — SubagentStatus 섹션(단독 서브에이전트, 6분 flush 공백 실측으로 10분 활동창), 고정 🧵 fab 토글 + 오버뷰 모드, 실토큰 테마 정합(구 스타일은 미존재 변수의 다크 폴백에 고정돼 라이트에서도 다크). 실행 중인 실제 위임의 라이브 데이터 + 테마 전환 실측 포함 E2E. verify-nway 20축 PASS.
