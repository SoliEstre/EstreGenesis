...
**Current**: v2.5.191 (2026-07-20) — **Adopter-driven triple fix (Constellation v2.4.78, plugin v0.3.31)** — a peer-main adopter's re-sync dogfood surfaced three measured gaps, all fixed: the MCP proxy gains the peer-key path (`CONSTELLATION_PEER_KEY` → `?peerKey=`, closing a §13.16.11 conflict where a peer had to ride the upstream parameter), the board-state serve path gains a machine gate (parse-validate + last-known-good fallback — one trailing comma had blanked every panel and poisoned the backup), and the bridge's empty `onerror` now closes into the reconnect chain (measured 43h symptomless hub absence). verify-nway 20-axis PASS.
