...
<p align="center">
  <img alt="Version: v2.5.46" src="https://img.shields.io/badge/version-v2.5.46-2ea44f?style=for-the-badge" />
...
**Current**: v2.5.46 (2026-06-04) — `@profile` explicit cut applied to 11 previously-unspecified `constellation/*.eux` files, closing the G8b drift-check warn class (`@profile`-unspecified + `ports.in`-present → semantics-undecided warn) introduced by EstreUX `7a4b591`. All 21 `constellation/*.eux` files now carry explicit `@profile` after this cut: `backend-service` 7 (server family 5 + history-store + ws-core) · `protocol-adapter` 6 (gateway-client family 5 + local-bridge) · `ui-component` 6 (ws-channel-input + ws-conn-bar + ws-tabs + ws-tool-card + ws-fab-badge + ws-collab-invite) · `supervisor` 2 (self-wake-watcher + watchdog). `ws-core` placed in `backend-service` per its `@intent` body ("deps-0 raw WebSocket transport core sitting below `server.cjs`, server-side library, pure transport, no DOM") — deviates from hub initial mapping `ws-* 7개=ui-component`, reversible at v2.5.47 if hub disagrees on review. No protocol wire change; one `@profile` line added per file; body unchanged; zero regression. Seed-tier (Master + Lite + Compact × EN/KO) is at **v2.4.3** (2026-06-03). Most recent module cuts: Constellation v2.3.22, Superscalar v0.4.2, Hyperbrief v0.5.6.
...
**현재**: v2.5.46 (2026-06-04) — 11 미지정 `constellation/*.eux` 파일에 `@profile` 명시 추가 cut, EstreUX `7a4b591` 가 도입한 G8b drift-check warn 클래스 (`@profile` 미지정 + `ports.in` 보유 → semantics 미결정 warn) 해소. 본 cut 후 모든 21 `constellation/*.eux` 파일이 명시 `@profile` 보유: `backend-service` 7 (server family 5 + history-store + ws-core) · `protocol-adapter` 6 (gateway-client family 5 + local-bridge) · `ui-component` 6 (ws-channel-input + ws-conn-bar + ws-tabs + ws-tool-card + ws-fab-badge + ws-collab-invite) · `supervisor` 2 (self-wake-watcher + watchdog). `ws-core` 는 `@intent` 본문 ("deps-0 raw WebSocket transport core, `server.cjs` 하위에 위치한 server-side 라이브러리, pure transport, DOM 없음") 에 따라 `backend-service` 배치 — 허브 초기 매핑 `ws-* 7개=ui-component` 에서 벗어남, 허브 review 시 disagree 면 v2.5.47 에서 수정 가능. 프로토콜 wire 변경 없음; 파일당 `@profile` 1줄 추가; 본체 무변경; 회귀 0. 시드 tier (Master + Lite + Compact × EN/KO) 는 **v2.4.3** (2026-06-03). 최근 모듈 cut: Constellation v2.3.22, Superscalar v0.4.2, Hyperbrief v0.5.6.
