...
<p align="center">
  <img alt="Version: v2.5.48" src="https://img.shields.io/badge/version-v2.5.48-2ea44f?style=for-the-badge" />
...
## Beyond bootstrap — three optional modules

v1.x is the project seed — bootstrap a new AI-native project, or migrate an existing one, into the `AGENTS.md` SSoT (everything above). **v2.0+ adds three optional modules** layered on top:
...
All three modules are **optional** and **referenced** (not bundled into the seed tiers, so the tier seeds stay lean). File-based coordination (Phase 5) remains the default and is enough for most projects.

### Install as Claude Code plugins (self-hosted marketplace)

The same three modules also ship as Claude Code plugins via a **self-hosted marketplace** in this repo. From any Claude Code session:
...
Each plugin is independent — install one, two, or all three. The marketplace metadata lives at [.claude-plugin/marketplace.json](.claude-plugin/marketplace.json); each plugin's source is under [plugins/](plugins/). Apache-2.0. The plugins are Phase 2 production-ready (Constellation v0.2.3 + MCP server with 5 tools; Superscalar v0.1.2 + §3.1 Hyperbrief interlock; Hyperbrief v0.5.6 + 3 skills + MCP server with 4 tools + PreToolUse/Stop hooks). Anthropic-side community-marketplace listing is deferred to v1.0 GA per [Hyperbrief.md §11.5](Hyperbrief.md) readiness rubric Lens B.
...
**Current**: v2.5.48 (2026-06-04) — phase_3 P3 2nd cut: `@invariants` + `@metamorphic` 절 격상 to `server-keys.eux` + `server-core.eux` + `server-history.eux` per eux-format v1.2 §2.6/§2.7 4 sub-sections (state/temporal/transaction/causality, EstreUX `edd2fb7`). Natural continuation of the 1st cut (v2.5.47) — same prose+vocabulary form, same 17-keyword retention pattern. **All 5 1st-tier readiness targets now carry `@invariants` + `@metamorphic`** after this cut: history-store + server-relay (1st cut) + server-keys + server-core + server-history (2nd cut). `server-keys.eux` carries KEY-MGMT credential lifecycle invariants (revoke persist-before force-close · KeyIssued at-most-once delivery + expires-after immediate · ws-keys.json atomic rename rollback-safe · wsKeyRole upstream iff uk- + ¬revoked). `server-core.eux` carries HTTP+SSE+WS transport invariants (3-frame upgrade commit-then sequence · serveFeedback atomic rollback-safe · MAX_BODY=32MB within request · envelope mismatch implies silent dashboard drop). `server-history.eux` carries persistence + lifecycle invariants (wsHistByChan HIST_CAP=200 bounded · history_lifecycle member-of {active,cold,archived} · timestamp_invariant persist-before · wsLoadAll atomic rollback-safe). Body prose unchanged; +125 lines added across 3 files. `@machine wsWaitState` (§13.19) formal lift still deferred to a separate cut (v2.5.49+). Seed-tier (Master + Lite + Compact × EN/KO) is at **v2.4.3** (2026-06-03). Most recent module cuts: Constellation v2.3.22, Superscalar v0.4.2, Hyperbrief v0.5.6.
...
v1.x 는 프로젝트 시드 — 신규 AI-Native 프로젝트 부트스트랩, 또는 기존 프로젝트를 `AGENTS.md` SSoT 로 마이그레이션 (위의 모든 내용). **v2.0+ 는 그 위에 3개의 선택 모듈 추가**:
...
세 모듈 모두 **선택적** + **참조** (시드 티어 본문에 번들되지 않으므로 시드는 lean 유지). 파일 기반 코디네이션 (Phase 5) 이 기본이며 대부분 프로젝트에 충분.

### Claude Code 플러그인으로 설치 (repo 자체 마켓플레이스)

세 모듈은 본 repo 의 **repo 자체 마켓플레이스** 를 통해 Claude Code 플러그인으로도 ship. Claude Code 세션에서:
...
각 플러그인 독립 — 하나, 둘, 또는 셋 모두 설치 가능. 마켓플레이스 메타데이터는 [.claude-plugin/marketplace.json](.claude-plugin/marketplace.json), 각 플러그인 소스는 [plugins/](plugins/) 아래. Apache-2.0. Phase 2 production-ready (Constellation v0.2.3 + 5-tool MCP 서버; Superscalar v0.1.2 + §3.1 Hyperbrief interlock; Hyperbrief v0.5.6 + 3 skill + 4-tool MCP 서버 + PreToolUse/Stop hook). Anthropic-side community-marketplace 등록은 [Hyperbrief.md §11.5](Hyperbrief.md) readiness rubric Lens B 에 따라 v1.0 GA 까지 defer.
...
**현재**: v2.5.48 (2026-06-04) — phase_3 P3 2nd cut: `server-keys.eux` + `server-core.eux` + `server-history.eux` 에 `@invariants` + `@metamorphic` 절 격상 per eux-format v1.2 §2.6/§2.7 4 sub-section (state/temporal/transaction/causality, EstreUX `edd2fb7`). 1st cut (v2.5.47) 의 자연 후속 — 동일 prose+vocabulary 형식 + 동일 17 키워드 잔존 패턴. **본 cut 후 1st-tier readiness 5 대상 모두 `@invariants` + `@metamorphic` 보유**: history-store + server-relay (1st cut) + server-keys + server-core + server-history (2nd cut). `server-keys.eux` 가 KEY-MGMT credential lifecycle invariants 운반 (revoke persist-before force-close · KeyIssued at-most-once delivery + expires-after immediate · ws-keys.json atomic rename rollback-safe · wsKeyRole upstream iff uk- + ¬revoked). `server-core.eux` 가 HTTP+SSE+WS transport invariants 운반 (3-frame upgrade commit-then sequence · serveFeedback atomic rollback-safe · MAX_BODY=32MB within request · envelope mismatch implies silent dashboard drop). `server-history.eux` 가 persistence + lifecycle invariants 운반 (wsHistByChan HIST_CAP=200 bounded · history_lifecycle member-of {active,cold,archived} · timestamp_invariant persist-before · wsLoadAll atomic rollback-safe). 본체 prose 무변경; 3 파일 +125줄 추가. `@machine wsWaitState` (§13.19) 정식 격상은 여전히 별도 cut 보류 (v2.5.49+). 시드 tier (Master + Lite + Compact × EN/KO) 는 **v2.4.3** (2026-06-03). 최근 모듈 cut: Constellation v2.3.22, Superscalar v0.4.2, Hyperbrief v0.5.6.
