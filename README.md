...
**Current**: v2.5.180 (2026-07-19) — **ops status strip + OpsState (Constellation v2.4.71)** — composer left-end strip showing the target's model/effort/subscaler from change-triggered declarations, with honest dropdown controls (actionable only via manifest-declared commands). verify-nway 20-axis PASS.
...
**현재**: v2.5.180 (2026-07-19) — **입력줄 상태 스트립 + OpsState (Constellation v2.4.71)** — composer 왼쪽 끝에 타깃의 model/effort/subscaler 를 변경-트리거 선언으로 표시하고, 드랍다운 제어는 매니페스트 선언 명령 경로만 actionable 이에요. verify-nway 20축 PASS.
