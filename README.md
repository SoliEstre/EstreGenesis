...
<p align="center">
  <img alt="Version: v2.5.153" src="https://img.shields.io/badge/version-v2.5.153-2ea44f?style=for-the-badge" />
...
**현재**: v2.5.153 (2026-07-17) — **Constellation v2.4.53 — roundtable: 다자간 topic room (§13.30)** — 셋 이상의 화자(에이전트+사람)에게 1급 거처가 생겨요: **이층 분리** — 서버는 결정론적 floor 를 강제하고(기존 1:1 relay 위 room fan-out, 파킹하는 `autoHop` 체인 상한, rate+연속 발화 상한, stall 감지, 인간 soft-yield 우선권), 신규 **`/roundtable` 규율 스킬**이 판단 계층을 소유해요(침묵 기본값 게이팅, 지목 시 응답 의무, 발언권 bid, 고정 catch-up 순서, ack-by-reference). 어휘는 MUC/Matrix 계보(`voice`·`moderated`·`moderator`·`notice`) 정렬; 5축 딥리서치(121출처·적대 검증 12건)가 근거. 스펙-우선 컷 — 서버/대시보드 구현은 R2-R5 트랙. verify-nway 19축 PASS.
