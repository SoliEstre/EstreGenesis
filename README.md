...
<p align="center">
  <img alt="Version: v2.5.79" src="https://img.shields.io/badge/version-v2.5.79-2ea44f?style=for-the-badge" />
...
Previously: v2.5.75 (2026-06-11) — **전수점검 Phase 2 보안 패치 3건 (S6/S10/S11) — Constellation v2.4.13 · Hyperbrief v0.6.1 · Ultrasafe v0.2.2** — 전수점검(reports/2026-06-10-eg-full-audit/report.md) 뒷부분 재개에서 미처리 ☐ 보안 발견 중 패치 차원 3건 (보안=패치 진행 standing rule, push 게이트): **(1) S6 대시보드 decisions detail XSS** — `dashboard/app.js` 가 검토사안 `detail` 만 raw innerHTML(done/planned 의 detail 은 esc 되는 비대칭) → `esc(d.detail)`; `previewHtml`/`previewUrl` 은 의도적 viz 슬롯이라 raw 유지(검토사안 등재 권한이 신뢰 경계 — v2.5.72 operator-채널 판정 일관). inner+outer self-board 동기. **(2) S10 hyperbrief MCP path traversal** — `mcp/server.cjs` `resolveLedgerPath` 가 호출자 `ledger_path` 무검증 → cwd-confine + traversal/absolute reject (env/default 는 운영자 신뢰라 면제). **(3) S11 Ultrasafe state.json 비밀 평문** — `ultrasafe-trigger.cjs` 가 publish 명령 전문(≤200자)을 `.ultrasafe/state.json` 에 평문 저장(`--otp`/`NPM_TOKEN=` 노출) → `maskSecrets`(플래그/env-var 명 보존, 값만 `[redacted]`) + `.ultrasafe/` gitignore. **검증**: node --check 4파일 + maskSecrets 단위테스트 + `.ultrasafe` check-ignore PASS. **나머지 ☐ 판정**: S4(KeyList 평문)·S9(/api/feedback)는 v2.5.71 loopback bind 로 원격 차단·잔존 host-local 저위험(KeyList 마스킹은 wire 영향이라 S1 게이트 재설계와 별도), S5/S7/S8 은 v2.5.71-72 에서 기 차단 — report.md ☐→✅/판정 반영. **Module versions**: Constellation v2.4.12 → **v2.4.13** (plugin 0.3.13 — dashboard render only, **wire 무변경·서버 재기동 불필요**), Hyperbrief v0.6.0 → **v0.6.1**, Ultrasafe v0.2.1 → **v0.2.2**. Superscalar v0.1.2 unchanged. Seed-tier v2.4.3 unchanged. **No wire change**. verify-nway 5축 PASS. 전수점검 Phase 2 보안 검증+패치 완결 (U5 가지치기 verdict 교차검증 + U6 drift 전수는 잔여 단위). 전체 리포트: outer `/reports/2026-06-10-eg-full-audit/report.md`.
...
이전: v2.5.75 (2026-06-11) — **전수점검 Phase 2 보안 패치 3건 (S6/S10/S11) — Constellation v2.4.13 · Hyperbrief v0.6.1 · Ultrasafe v0.2.2** — 전수점검 뒷부분 재개에서 미처리 ☐ 보안 발견 중 패치 차원 3건 (보안=패치 진행, push 게이트): **(1) S6** 대시보드 검토사안 `detail` 만 raw innerHTML(done/planned 는 esc 되는 비대칭) → `esc(d.detail)`; `previewHtml`/`previewUrl` 은 viz 슬롯이라 raw 유지(inner+outer 동기). **(2) S10** hyperbrief MCP `resolveLedgerPath` 가 호출자 `ledger_path` 무검증 → cwd-confine + traversal/absolute reject. **(3) S11** Ultrasafe `state.json` 에 publish 명령 비밀 평문 저장(`--otp`/`TOKEN=`) → `maskSecrets`(값만 `[redacted]`) + `.ultrasafe/` gitignore. 검증: node --check + maskSecrets 테스트 + check-ignore PASS. 나머지 ☐(S4/S9)는 v2.5.71 loopback 으로 원격 차단·잔존 host-local, S5/S7/S8 기 차단 — report.md 반영. **모듈 version**: Constellation v2.4.12 → **v2.4.13**(plugin 0.3.13, dashboard render only, wire 무변경·재기동 불필요), Hyperbrief v0.6.0 → **v0.6.1**, Ultrasafe v0.2.1 → **v0.2.2**. 기타 변경 없음. 시드 tier v2.4.3 유지. **wire 변경 없음**. verify-nway 5축 PASS. 전수점검 Phase 2 보안 검증+패치 완결(U5 verdict 교차검증 + U6 drift 잔여). 리포트: outer `/reports/2026-06-10-eg-full-audit/report.md`.
