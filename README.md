...
**현재**: v2.5.166 (2026-07-19) — **인스펙터 XSS 핫픽스 (Constellation v2.4.62)** — 푸시-후 자동 보안 리뷰가 당일 검출: 신설 플로팅 인스펙터가 와이어-유래 WorkflowStatus 값을 innerHTML 헬퍼로 렌더(§8.1 위반, 접속 에이전트 주입 가능) → 전부 textContent 주입 + state 클래스 화이트리스트로 교정, 실브라우저 회귀(전 필드 악성 페이로드 → 이스케이프 텍스트·실행 0) 통과, 컷 전 라이브 배포. verify-nway 20축 PASS.
