...
**현재**: v2.5.192 (2026-07-20) — **peer-키 셀프 발급 경로 문서화 (Constellation v2.4.79)** — 어댑터 재동기가 v2.5.191 수정 3건 전량 드리프트-0 채택 + 신규 MCP 경로의 role=peer 라이브 검증으로 완결됐고, 코드를 읽어야만 알 수 있던 갭 1건이 남았어요: 헤드리스 어댑터가 자기 pk- 키를 발급하려면 board 연결(무-HELLO)을 써야 해요(키 관리가 agent 표면에서만 main-게이트라서). §13.16.11 에 그 경로를 명시했어요. verify-nway 20축 PASS.
