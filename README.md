...
**Current**: v2.5.172 (2026-07-19) — **context-threshold handoff-guard (Constellation v2.4.68)** — at ≥75% context (configurable) a Stop hook requires the compact-handoff card updated every turn until compaction (one block-nudge per turn, auto-disarm on compact, transcript-measured usage). verify-nway 20-axis PASS.
...
**현재**: v2.5.172 (2026-07-19) — **컨텍스트 임계 handoff-guard (Constellation v2.4.68)** — 컨텍스트 75%(설정 가능) 이상이면 compact 까지 매 턴 handoff 카드 갱신을 요구해요(턴당 1회 nudge·compact 시 자동 해제·transcript 실측). verify-nway 20축 PASS.
