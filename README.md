...
<p align="center">
  <img alt="Version: v2.5.146" src="https://img.shields.io/badge/version-v2.5.146-2ea44f?style=for-the-badge" />
...
Seven plugins: the **estregenesis** kit (the seed itself, as `/egboot` · `/egmig` · `/egup` · `/egrich`) plus the six modules. Each is independent — install one, some, or all of them. The marketplace metadata lives at [.claude-plugin/marketplace.json](.claude-plugin/marketplace.json); each plugin's source is under [plugins/](plugins/). Apache-2.0. Current: estregenesis v0.1.0 (4 procedures + 3 pointer aliases, 0 deps); Constellation v0.3.23 (+ 5-tool MCP server, Stop hook); Superscalar v0.1.2 (+ §3.1 Hyperbrief interlock); Hyperbrief v0.7.1 (+ 3 skills, 4-tool MCP server, PreToolUse/Stop hooks); Greatpractice v0.3.4 (+ `/routinize`, 3 JSON schemas, 1 contact hook); Ultrasafe v0.2.4 (+ 8-agent red-team fan-out, clean-signal gate — advisory, never blocks a publish); Compendium v0.2.6 (+ 8-tool MCP server, gardening lint, 2 cross-tool skills). Anthropic-side community-marketplace listing is deferred to v1.0 GA per [Hyperbrief.md §11.5](Hyperbrief.md) readiness rubric Lens B.
...
**Current**: v2.5.146 (2026-07-13) — **`estregenesis` — the kit plugin: EG itself as commands** — adopting EG no longer means hunting a raw GitHub URL. Four procedures: `/egboot` (new project) · `/egmig` (existing rule files — every rule promoted or dropped-with-reason) · `/egup` (seed + plugin delta, additive, local edits win) · `/egrich` (full arm, cost stated up front). Each routes to the right sibling on misclassification; the three short names are **pointer aliases, not copies** (Compendium's pointer-not-paraphrase applied to the plugin's own surface). The skills fetch and run the seed — never restate it, never reconstruct an unfetchable one from memory. 0 deps, no runtime. **Promo sweep**: the quickstart now leads with the one-command path; the Compendium card had been advertising "v0.1.0, design draft" five releases after its runtime shipped; the plugin roster was frozen at Constellation v0.3.16; `docs/index.html` gained a kit card. **verify-nway hole closed**: the `readme-current` axis matched only the first marker per language — two later 버전 관리 copies had been frozen at v2.5.137 for 8 cuts while it reported PASS. Now checks every marker. 19 axes, 0 drift.

Previously: v2.5.145 (2026-07-12) — **Constellation v2.4.51 — reference bridge commitment-ack default-on** — the other half of the redelivery fix. The reference **server** starts the §13.13.2 redelivery scheduler unconditionally, while the reference **bridge**'s delivered-persist ack sat behind an opt-in `RELAY_REDELIVERY=on` — so a stock server+bridge deployment redelivered every targeted msgId CUSTOM 3× and closed each with a false `RelayUnreachable{commitment-ack-absent}`. Gate inverted (ack on by default; broker-forward deployments opt out with `RELAY_REDELIVERY=off`). With v2.4.50's worker leg the loop closes in both directions — live-verified (probe each way → ack → zero redelivery). verify-nway 18-axis PASS.

Previously: v2.5.144 (2026-07-12) — **Ultrasafe v0.2.4 — hook state-anchor fix** — both hooks picked `.ultrasafe/` from the tool call's cwd, so a session that `cd`s into a nested repo wrote its release events into a second state file and any single-file read undercounted the evidence (EG dogfood 2026-07-11: 5 visible, 13 actual). Since the advisory→blocking (v0.3) transition is gated on that count, a split ledger biases it downward. Both hooks now anchor to a stable root (`CLAUDE_PROJECT_DIR` → `ULTRASAFE_REPO_ROOT` → cwd-walk), pipe-tested from a nested cwd; EG's split ledger merged (13 events). verify-nway 18-axis PASS.

Previously: v2.5.143 (2026-07-12) — **Constellation v2.4.50 — reference `join-local` commitment-ack conformance** — the reference worker client never sent the §13.13.2 commitment-tier ack, so every targeted msgId CUSTOM was redelivered 3× and closed with `RelayUnreachable{commitment-ack-absent}` noise per delegation (measured 2026-07-04~11). Blocker re-examination found the contract entirely EG-owned (no hub inquiry needed): `join-local` now replies `AckProcessed{ackFor}` on receipt (ack/ping kinds excluded — storm-safe), clearing the server's at-least-once pending at first delivery. Live-verified end-to-end. verify-nway 18-axis PASS.

Previously: v2.5.142 (2026-07-11) — **`.eux` line-ending pin (`.gitattributes`, reverse-sync e2e-001 f3 prevention)** — a Windows-autocrlf checkout materializes `.eux` specs as CRLF and single-line `(.+)$`-style parsers then silently extract zero clauses (observed live on the hub's first pull of `history-store.eux`; latent EG-side). The runner fix shipped hub-side; EG's prevention axis pins `*.eux text eol=lf` at the repo root (the v2.4.47 runtime-script lesson extended to the spec surface) + renormalizes the one drifted worktree copy. verify-nway 18-axis PASS.

Previously: v2.5.141 (2026-07-11) — **P4b dynamic-gate anchor: `constellation/history-store.p4.mjs` (reverse-sync B3 pilot — the EG-owned P4 leg goes live)** — the metamorphic-property anchor the reverse-sync verdict spec (v0.2) assigns to EG: loaded by the EstreUX runner (`p4-check --run`) via the P4b contract (`run(props)` export), it executes `history-store.eux`'s 7 `@metamorphic` clauses against the minimal Mode-A reference impl with fixed-seed fast-check (cross-side reproducible verdicts). Coverage-honest wiring: 3 clauses direct, 3 as declared Mode-A projections of Mode-B clauses (projection notes in-output), 1 explicit SKIP (archive surface absent in the minimal impl — disclosed, never silent); unmapped clauses are conservative FAILs; `EUX_P4_IMPL`/`EUX_P4_RUNS` env hooks let the e2e run mutated implementations. Verified three-legged (positive 6-attested/exit 0 · destructive mutation fails exactly its clause/exit 1 · unknown clause fails unmapped/exit 1). Dev-dep fast-check via new `constellation/package.json` (runtime stays dependency-zero). verify-nway 18-axis PASS.
...
플러그인 7종: **estregenesis** 킷(시드 자체를 `/egboot` · `/egmig` · `/egup` · `/egrich` 로) + 6개 모듈. 각각 독립이라 하나만, 몇 개만, 또는 전부 설치 가능. 마켓플레이스 메타데이터는 [.claude-plugin/marketplace.json](.claude-plugin/marketplace.json), 각 플러그인 소스는 [plugins/](plugins/) 아래. Apache-2.0. 현재: estregenesis v0.1.0 (절차 4종 + 포인터 별칭 3종, 의존성 0); Constellation v0.3.23 (+ 5-tool MCP 서버, Stop 훅); Superscalar v0.1.2 (+ §3.1 Hyperbrief interlock); Hyperbrief v0.7.1 (+ 스킬 3종, 4-tool MCP 서버, PreToolUse/Stop 훅); Greatpractice v0.3.4 (+ `/routinize`, JSON 스키마 3종, contact 훅 1종); Ultrasafe v0.2.4 (+ 8-agent red-team fan-out, clean-signal 게이트 — advisory, 출시를 막지 않음); Compendium v0.2.6 (+ 8-tool MCP 서버, 가드닝 lint, cross-tool 스킬 2종). Anthropic-side community-marketplace 등록은 [Hyperbrief.md §11.5](Hyperbrief.md) readiness rubric Lens B 에 따라 v1.0 GA 까지 defer.
...
**현재**: v2.5.146 (2026-07-13) — **`estregenesis` — 킷 플러그인: EG 자체를 명령으로** — 이제 EG 도입에 raw 깃허브 URL 을 찾아다닐 일이 없음. 절차 4종: `/egboot` (신규 프로젝트) · `/egmig` (기존 룰 파일 — 모든 룰을 승격하거나 사유와 함께 폐기) · `/egup` (시드 + 플러그인 델타, 가산형, 로컬 편집이 이김) · `/egrich` (풀 무장, 비용 선고지). 오분류 시 맞는 형제로 라우팅하고, 짧은 이름 3종은 **복사본이 아니라 포인터 별칭** (Compendium 의 pointer-not-paraphrase 를 플러그인 자기 표면에 적용). 스킬은 시드를 가져와 실행할 뿐 재진술하지 않고, 가져올 수 없는 시드를 기억으로 재구성하지 않음. 의존성 0, 런타임 없음. **promo 훑기**: 60초 시작이 명령 하나 경로를 먼저 제시; Compendium 카드가 런타임 출시 5릴리스 뒤에도 "v0.1.0, design draft" 광고 중이었음; 플러그인 로스터가 Constellation v0.3.16 에 동결; `docs/index.html` 에 킷 카드 추가. **verify-nway 구멍 해소**: `readme-current` 축이 언어별 첫 마커만 매칭 — 뒤쪽 버전 관리 사본 2벌이 v2.5.137 에 8컷 동결인데도 PASS 였음. 이제 전 마커 검사. 19축, 드리프트 0.

이전: v2.5.145 (2026-07-12) — **Constellation v2.4.51 — 레퍼런스 브릿지 commitment-ack 기본 on** — 재전달 수정의 나머지 절반. 레퍼런스 **서버**는 §13.13.2 재전달 스케줄러를 무조건 기동하는데 레퍼런스 **브릿지**의 delivered-persist ack 은 opt-in `RELAY_REDELIVERY=on` 뒤에 있어서, 기본 서버+브릿지 배치가 모든 targeted msgId CUSTOM 을 3× 재전달하고 false `RelayUnreachable{commitment-ack-absent}` 로 종결했음. 게이트 반전(ack 기본 on; broker-forward 배치가 `RELAY_REDELIVERY=off` 로 opt-out). v2.4.50 워커 다리와 합쳐 양방향 완결 — 라이브 검증(양방향 프로브 → ack → 재전달 0). verify-nway 18축 PASS.

이전: v2.5.144 (2026-07-12) — **Ultrasafe v0.2.4 — 훅 상태파일 앵커 수정** — 두 훅이 `.ultrasafe/` 를 툴 호출의 cwd 로 골라서, 중첩 저장소로 `cd` 한 세션은 릴리스 이벤트를 두 번째 상태파일에 쌓았고 한쪽만 읽으면 증거가 과소집계됐음 (EG 도그푸드 2026-07-11: 보이는 5건, 실제 13건). advisory→blocking(v0.3) 전환이 이 카운트로 게이팅되므로 갈라진 원장은 전환을 아래로 편향시킴. 이제 두 훅 모두 안정 루트 앵커(`CLAUDE_PROJECT_DIR` → `ULTRASAFE_REPO_ROOT` → cwd-walk), 중첩 cwd 파이프 테스트 확인; EG 의 갈라진 원장 병합(13건). verify-nway 18축 PASS.

이전: v2.5.143 (2026-07-12) — **Constellation v2.4.50 — reference `join-local` commitment-ack 정렬** — 레퍼런스 워커 클라이언트가 §13.13.2 commitment-tier ack 을 안 보내서 매 targeted msgId CUSTOM 이 3× 재전달 + `RelayUnreachable{commitment-ack-absent}` 소음으로 종결되던 것(2026-07-04~11 실측). blocker 재검증에서 계약이 전부 EG 소유임이 확인돼(허브 문의 불요) unblock: `join-local` 이 수신 즉시 `AckProcessed{ackFor}` 회신(ack/ping 류 제외 — 스톰 안전), 서버 at-least-once pending 을 1차 전달에서 clear. 라이브 end-to-end 검증. verify-nway 18축 PASS.

이전: v2.5.142 (2026-07-11) — **`.eux` 라인엔딩 고정 (`.gitattributes`, reverse-sync e2e-001 f3 재발 방지)** — Windows autocrlf 체크아웃이 `.eux` 를 CRLF 로 물질화하면 라인단위 `(.+)$` 계열 파서가 절 추출 0건으로 조용히 죽음 (허브의 `history-store.eux` 첫 pull 에서 실발현; EG 측은 로컬 실행이 우연히 LF 사본을 타서 잠복). 러너 수정은 허브 측 출시; EG 예방축으로 루트에 `*.eux text eol=lf` 고정(v2.4.47 런타임 스크립트 교훈의 스펙 표면 확장) + 드리프트된 워크트리 사본 1건 재정규화. verify-nway 18축 PASS.

이전: v2.5.141 (2026-07-11) — **P4b 동적 게이트 앵커: `constellation/history-store.p4.mjs` (reverse-sync B3 파일럿 — EG 소관 P4 레그 가동)** — reverse-sync 판정 규격(v0.2)이 EG 몫으로 배정한 metamorphic-property 앵커: EstreUX 실행기(`p4-check --run`)가 P4b 계약(`run(props)` export)으로 로드, `history-store.eux` 의 `@metamorphic` 7개 절을 minimal Mode-A 레퍼런스 구현에 고정 seed fast-check 로 실행(양측 재현 가능 판정). 커버리지 정직 배선: 3개 절 direct, 3개는 Mode-B 절의 Mode-A projection 선언 실행(출력에 projection 주석), 1개 명시 SKIP(minimal 구현에 archive 표면 부재 — 공개 갭, silent 아님); 미매핑 절 = 보수적 FAIL; `EUX_P4_IMPL`/`EUX_P4_RUNS` env 훅으로 e2e 가 변형 구현 실행 가능. 3중 검증(정상 6-attested/exit 0 · 파괴 변형은 해당 절만 FAIL/exit 1 · 미지 절 unmapped FAIL/exit 1). dev-dep fast-check 는 신규 `constellation/package.json`(런타임 의존성 0 유지). verify-nway 18축 PASS.
