...
<p align="center">
  <img alt="Version: v2.5.49" src="https://img.shields.io/badge/version-v2.5.49-2ea44f?style=for-the-badge" />
...
**Current**: v2.5.49 (2026-06-04) — `constellation/reference/runtime/history-store.cjs` minimal reference impl ship (P4 dogfood prerequisite): Mode A `JsonlStore` floor + `HistoryStoreMux` mode-routing + `createHistoryStore` factory + `msgChan` content-derived key helper + `HIST_CAP=200` + `SKIP_NAMES` (~140 lines, Apache-2.0). Implements the surface that `history-store.eux` `@metamorphic` 7 properties (round_trip 3 + idempotency 2 + determinism 2, v2.5.47 1st cut) target — `JsonlStore.exportJsonl()` ordering (ts ASC, id ASC + tie-break) satisfies `round_trip` byte-identity · `JsonlStore.boot()` content-derived re-key satisfies `idempotency` boot reload · `JsonlStore.append()` additive ring satisfies `determinism`. EstreUX `c6989b8` `p4-check --run` can wire fast-check properties against this impl. Mode B/C dormant by design — any `opts.mode !== 'jsonl'` falls back to Mode A with advisory BlockerManifest; full Mode B/C impl (SqliteStore + selectSqliteBackend + backfillFromJsonl + reconcile + auto-revert + WAL recovery) deferred to a separate cycle. Seed-tier (Master + Lite + Compact × EN/KO) is at **v2.4.3** (2026-06-03). Most recent module cuts: Constellation v2.3.22, Superscalar v0.4.2, Hyperbrief v0.5.6.
...
**현재**: v2.5.49 (2026-06-04) — `constellation/reference/runtime/history-store.cjs` minimal reference impl ship (P4 dogfood prerequisite): Mode A `JsonlStore` floor + `HistoryStoreMux` mode-routing + `createHistoryStore` factory + `msgChan` content-derived key helper + `HIST_CAP=200` + `SKIP_NAMES` (~140줄, Apache-2.0). `history-store.eux` `@metamorphic` 7 property (round_trip 3 + idempotency 2 + determinism 2, v2.5.47 1st cut) 가 target 으로 하는 surface 구현 — `JsonlStore.exportJsonl()` ordering (ts ASC, id ASC + tie-break) 가 `round_trip` byte-identity 충족 · `JsonlStore.boot()` content-derived re-key 가 `idempotency` boot reload 충족 · `JsonlStore.append()` additive ring 이 `determinism` 충족. EstreUX `c6989b8` `p4-check --run` 가 fast-check property wiring 본 impl 에 가능. Mode B/C dormant by design — 모든 `opts.mode !== 'jsonl'` 은 Mode A fallback + advisory BlockerManifest; full Mode B/C impl (SqliteStore + selectSqliteBackend + backfillFromJsonl + reconcile + auto-revert + WAL recovery) 별도 cycle 보류. 시드 tier (Master + Lite + Compact × EN/KO) 는 **v2.4.3** (2026-06-03). 최근 모듈 cut: Constellation v2.3.22, Superscalar v0.4.2, Hyperbrief v0.5.6.
