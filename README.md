...
<p align="center">
  <img alt="Version: v2.5.151" src="https://img.shields.io/badge/version-v2.5.151-2ea44f?style=for-the-badge" />
...
**현재**: v2.5.151 (2026-07-17) — **P4 앵커: `EUX_P4_SEED` env 오버라이드** — `constellation/history-store.p4.mjs` 의 고정 fast-check seed 가 env 오버라이드 가능한 기본값이 돼요: `EUX_P4_SEED` 가 `EUX_P4_IMPL`/`EUX_P4_RUNS` 훅 계열에 합류 — reverse-sync e2e 라운드가 앵커 파일을 건드리지 않고 양측에 새 seed 를 고정할 수 있고, 기본값(`20260711`)은 과거 판정과의 비교 가능성을 유지해요. 양방향 검증: 기본 실행은 과거 판정 정확 재현(6 attested · 1 공개 SKIP · 0 failed), 오버라이드 실행은 새 seed 반영 + 전 절 attested. verify-nway 19축 PASS.
