...
**Current**: v2.5.171 (2026-07-19) — **prompt-injection slash-command autocomplete (Constellation v2.4.67)** — CLI-palette-style `/` autocomplete in the realtime composer (ESC closes the list only) + harness-agnostic `CommandManifest` declaration; backfills the v2.5.169/170 changelog rows + adds the reverse marker-presence check. verify-nway 20-axis PASS.
...
**현재**: v2.5.171 (2026-07-19) — **주입행 슬래시 명령 자동완성 (Constellation v2.4.67)** — 실시간 composer 의 CLI 팔레트식 `/` 자동완성(ESC 는 후보만 닫아요) + 하네스 무관 `CommandManifest` 선언; v2.5.169/170 changelog 행 백필 + 역방향 마커 검사 추가. verify-nway 20축 PASS.
