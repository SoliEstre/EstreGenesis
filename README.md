...
**현재**: v2.5.194 (2026-07-20) — **선언 가이드 완결성 + 에코 행 (Constellation v2.4.81)** — 운영자의 충분성 질문이 가이드 갭 4건을 표면화, 전부 해소: §13.23.4 에 구체 재선언 트리거(커버 상태 전이마다 + 재접속마다) 명시, /subscaler 에 OpsState 재선언 단계 내장, BOOTSTRAP 에 선언-이벤트 3종 절 신설, 어댑터 계약이 1종 대신 3종 전부를 명명해요. 운용 드랍다운에 에코 행도 추가돼요(EchoModeState 소스, 매니페스트-게이트 /echo-mode 제어). verify-nway 20축 PASS.
