...
<p align="center">
  <img alt="Version: v2.5.160" src="https://img.shields.io/badge/version-v2.5.160-2ea44f?style=for-the-badge" />
...
**현재**: v2.5.160 (2026-07-18) — **지원 표에 Kimi Code 실명화 (doc-only)** — 실측: Kimi Code CLI 는 프로젝트 루트 `AGENTS.md` 를 1급 지시 파일로 취급해요(자체 `/init` 이 이를 생성) — EG 코어 층은 폴백 행으로 이미 커버였고, 이제 그 노트와 함께 실명 등재(EN + KO ×3). 스킬+MCP+훅 스택이라 어댑터 계약 Stage 1 후보로는 Codex 보다 강함 — 큐 기록만, 출시 아님. 시드 무변경. verify-nway 19축 PASS.
