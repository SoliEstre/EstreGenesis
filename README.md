...
**Current**: v2.5.179 (2026-07-19) — **timeout-composed speculation (Superscalar v0.7.0 §4.1, TC6)** — clarify-loose (sentinel self-proceed) vs approval-strict (fail-closed) gate polarity, speculative pre-execution during clarification waits, late answers as steering; risk actions never pre-executed. verify-nway 20-axis PASS.
...
**현재**: v2.5.179 (2026-07-19) — **타임아웃-합성 투기 (Superscalar v0.7.0 §4.1, TC6)** — 분기 게이트=느슨(sentinel 자체진행) vs 위험 게이트=엄격(fail-closed) 극성 + 분기 대기 중 유력 브랜치 선실행 + 늦은 답=스티어링; 위험 액션은 선실행 금지예요. verify-nway 20축 PASS.
