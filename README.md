...
**현재**: v2.5.190 (2026-07-20) — **선택지 응답 라우팅 + 위조 방지 (Constellation v2.4.77)** — 후보-호스트 보안 리뷰가 지목한 선택지 시설 경화 4항: 무타깃 응답은 main 폴백 대신 pending 발신자에게(at-least-once msgId 동반), 늦은 응답은 bounded tombstone 으로 발신자 라우팅, 서버/사람 전용 Selection 이름의 에이전트 발신은 위조로 차단, pending 발신자는 인증된 연결 identity 로 기록해요. verify-nway 20축 PASS.
